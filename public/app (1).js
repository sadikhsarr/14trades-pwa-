...
async function requestPush() {
  if (!('Notification' in window)) { showToast('Push not supported on this browser', 'info'); return; }
...
function startNextWindowCountdown() {
  updateNextWindow();
...
function updateNextWindow() {
  ...
  WINDOWS.forEach(w => {
    ...
    const wSecs = wH * 3600 + wM * 60;
    let diff = wSecs - nowSecs;
  ...
  document.getElementById('nw-countdown').textContent = display;
  document.getElementById('nw-window').textContent = best.time + ' ' + best.name;
  document.getElementById('nw-asset').textContent = best.asset;
  document.getElementById('nt-sub').textContent = 'Signal fires at ' + best.time + ' NY';

  // Update FOMO next time
  document.getElementById('fomo-next').textContent = 'Next signal in ' + (h > 0 ? h + 'h ' : '') + m + 'm';

  // Urgent — under 5 minutes
  const cd = document.getElementById('nw-countdown');
  if (best.secsUntil < 300) {
    cd.style.color = 'var(--yellow)';
  } else {
    cd.style.color = 'var(--white)';
  }
}

// ─── Market Mood ──────────────────────────────────────────────────
function startMarketMood() {
  updateMood();
  setInterval(updateMood, 60000); // update every minute
}

function updateMood() {
  const nyHour = parseInt(new Date().toLocaleTimeString('en-US', {
    timeZone: 'America/New_York', hour12: false, hour: '2-digit'
  }));

  // Session mood based on NY time
  let session, sessionClass;
  if (nyHour >= 8 && nyHour < 12) {
    session = 'NY Open'; sessionClass = 'bull';
  } else if (nyHour >= 12 && nyHour < 14) {
    session = 'NY Prime'; sessionClass = 'volatile';
  } else if (nyHour >= 14 && nyHour < 17) {
    session = 'NY Close'; sessionClass = 'bull';
  } else if (nyHour >= 2 && nyHour < 8) {
    session = 'London'; sessionClass = 'bull';
  } else if (nyHour >= 19 || nyHour < 2) {
    session = 'Asia'; sessionClass = 'neutral';
  } else {
    session = 'Off Hours'; sessionClass = 'neutral';
  }

  // Gold mood — rotate through realistic moods based on time
  const goldMoods = [
    { tag: 'Bullish', cls: 'bull' },
    { tag: 'Bearish', cls: 'bear' },
    { tag: 'Ranging', cls: 'neutral' },
    { tag: 'Volatile', cls: 'volatile' },
  ];
  const btcMoods = [
    { tag: 'Bullish', cls: 'bull' },
    { tag: 'Bearish', cls: 'bear' },
    { tag: 'Sideways', cls: 'neutral' },
    { tag: 'Pumping', cls: 'volatile' },
  ];

  // Use last signal from history to determine mood
  ...
  let goldMood = { tag: 'Watching', cls: 'neutral' };
  let btcMood  = { tag: 'Watching', cls: 'neutral' };

  if (lastGold) {
    goldMood = lastGold.direction === 'BUY'
      ? { tag: 'Bullish', cls: 'bull' }
      : { tag: 'Bearish', cls: 'bear' };
  }
  if (lastBtc) {
    btcMood = lastBtc.direction === 'BUY'
      ? { tag: 'Bullish', cls: 'bull' }
      : { tag: 'Bearish', cls: 'bear' };
  }
...
function checkFOMO(signals) {
  ...
  // Find most recent closed signal
  const closed = signals.find(s =>
    (s.type === 'tp_hit' || s.type === 'sl_hit') &&
    (Date.now() - new Date(s.timestamp)) < 3600000 // within last hour
  );
  ...
  if (closed) {
    ...
    document.getElementById('fomo-title').textContent = isTP
      ? `TP${closed.level} Hit — ${closed.asset} ${closed.direction}`
      : `SL Hit — ${closed.asset} ${closed.direction}`;
    document.getElementById('fomo-desc').textContent = isTP
      ? `${closed.asset} hit Take Profit ${closed.level}. Don't miss the next one.`
      : `Trade closed at Stop Loss. Stay ready for the next setup.`;
  } else {
    fomoCard.style.display = 'none';
    noTrade.style.display  = 'flex';
  }
...
function renderHistory(signals) {
  ...
  if (!items.length) {
    feed.innerHTML = '<div style="font-size:11px;color:var(--muted);padding:8px 2px">No history yet</div>';
    return;
  }
    ...
    return `
      <div class="hist-item">
        <div class="hi-left">
          ...
          <div>
            <div class="hi-asset">${sig.asset || 'GOLD'}</div>
