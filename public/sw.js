...
self.addEventListener('fetch', e => {
  ...
  // Always fetch JS and HTML fresh — never serve stale code
...
self.addEventListener('notificationclick', e => {
  ...
  e.waitUntil(
    clients.matchAll({ type: 'window', includeUncontrolled: true }).then(cls => {
      ...
      if (existing) {
        // Live notification — open TikTok directly even if app is open
        ...
        if (data.type === 'broadcast' && data.message) {
          existing.postMessage({ type: 'PUSH_RECEIVED', payload: { type: 'broadcast', message: data.message, bcType: data.bcType||'', bcLink: data.bcLink||'' } });
        }
        return;
      }
      ...
      let url = '/';
      if (data.type === 'live' && data.url) {
        // Open TikTok Live directly — not the app
        return clients.openWindow(data.url);
      } else if (data.type === 'prep') {
        url = `/?prep=${encodeURIComponent(JSON.stringify({type:data.type,secondsUntilSignal:data.secondsUntilSignal||60,id:data.id||Date.now().toString()}))}`;
      } else if (data.type === 'broadcast' && data.message) {
        url = `/?bc=${encodeURIComponent(data.message)}${data.bcType?'&bct='+encodeURIComponent(data.bcType):''}${data.bcLink?'&bcl='+encodeURIComponent(data.bcLink):''}`;
      }
