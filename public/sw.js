...
self.addEventListener('push', e => {
  ...
  const fullPayload = { ...data, ...(data.data || {}), type, asset, direction, secondsUntilSignal, id: pushId };
...
self.addEventListener('notificationclick', e => {
  ...
  e.waitUntil(
    clients.matchAll({ type: 'window', includeUncontrolled: true }).then(cls => {
      ...
      // Open app — pass prep data via URL query so countdown shows on open
      const url = data.type === 'prep'
        ? `/?prep=${encodeURIComponent(JSON.stringify({type:data.type,asset:data.asset,direction:data.direction,secondsUntilSignal:data.secondsUntilSignal||60,id:data.id||Date.now().toString()}))}`
        : '/';
