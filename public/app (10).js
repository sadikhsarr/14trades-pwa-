...
function showSignalCountdown(prep) {
  ...
  assetEl.textContent = asset + ' · XAU/USD';
  ...
  let secs = prep.secondsUntilSignal || 120;
  ...
  function tick() {
    ...
    timeEl.className   = secs <= 30 ? 'sco-time urgent' : 'sco-time';
    if (secs <= 30 && navigator.vibrate) navigator.vibrate(30);
    secs--;
  }
  tick();
...
document.addEventListener('visibilitychange', () => {
  if (document.visibilityState === 'visible') {
    lastTradeId = null; // reset so trade always shows fresh
    fetchState();
  }
});
...
function applyState({ activeTrade, signals, prepSignal }) {
  ...
  if (activeTrade) {
    if (activeTrade.id !== lastTradeId) {
      // New trade — show flash only if app was already open with a different trade
      if (lastTradeId !== null) {
        dismissCountdown();
    ...
    // ALWAYS update the live trade card — even if lastTradeId matches
