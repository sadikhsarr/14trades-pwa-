...
async function loadWeeklyRecap() {
  ...
  try {
    ...
    if (!r.ok) { console.error('[loadWeeklyRecap] HTTP error', r.status); return; }
    const d = await r.json();
    if (!d.ok || !Array.isArray(d.entries)) {
      console.error('[loadWeeklyRecap] Unexpected response shape', d);
      return;
    }
