...
function checkAccess() {
  ...
  if (saved && expiry > now) {
    ...
    hidePublicScreen();
    bootApp();
  } else if (saved && (expiry === 0 || isNaN(expiry))) {
    // Has code but no valid expiry — re-validate with server
...
async function revalidateCode(code) {
  try {
    ...
    const d = await r.json();
    if (d.valid) {
      // Still valid — restore expiry and boot
      const exp = d.expires_at ? new Date(d.expires_at).getTime() : Date.now() + 30*24*60*60*1000;
