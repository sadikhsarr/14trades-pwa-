...
function checkAccess() {
  // Just came from login page — boot directly, no checks needed
  if (sessionStorage.getItem('14t_just_logged_in') === '1') {
    sessionStorage.removeItem('14t_just_logged_in');
    hidePublicScreen();
    bootApp();
    return;
  }

  // Also check URL boot flag for older cached versions
  if (new URLSearchParams(window.location.search).get('boot') === '1') {
    window.history.replaceState({}, '', '/');
    hidePublicScreen();
    bootApp();
    return;
