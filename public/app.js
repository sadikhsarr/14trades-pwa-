...
function showBroadcastMessage(msg) {
  if (!msg) return;
  const ts = Date.now();
  // Save to localStorage — applyState will render it once app is visible
  localStorage.setItem('14t_broadcast', JSON.stringify({ msg, ts: String(ts), dismissed: false }));
  // Also try to render immediately if app is already visible
  const main = document.getElementById('main-app');
  if (main && main.style.display !== 'none') {
    renderBroadcastBanner(msg, ts);
  }
}
