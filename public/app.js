...
function renderBroadcastBanner(msg, ts) {
  ...
  if (card && msgEl) {
    ...
    card.style.cssText = 'display:block;background:#0D0C00;border:1px solid #C9A84C20;border-radius:12px;padding:12px 14px;position:relative;margin-bottom:4px';
    ...
    float.id = 'bc-float';
    float.style.cssText = 'background:#0D0C00;border:1px solid #C9A84C20;border-radius:12px;padding:12px 14px;margin:8px 14px 0;position:relative';
    float.innerHTML = `
      <div style="display:flex;align-items:flex-start;gap:10px">
        <div style="font-size:16px;flex-shrink:0">📢</div>
        <div style="flex:1">
          <div style="font-size:7px;font-weight:700;letter-spacing:1.5px;color:#C9A84C;text-transform:uppercase;margin-bottom:5px">📢 Message · Sadikh</div>
          <div style="font-size:13px;color:#F0F0F0;line-height:1.6">${msg}</div>
          <div style="font-size:8px;color:#C9A84C;opacity:.3;margin-top:4px">${ts ? getTimeAgo(ts) : ''}</div>
        </div>
