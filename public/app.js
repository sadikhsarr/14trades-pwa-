...
function showRenewalBanner(daysLeft) {
  ...
  if (!existing) {
    ...
    banner.style.cssText = 'background:#1A0D00;border-bottom:1px solid #F5A80030;padding:10px 16px;display:flex;align-items:center;gap:10px;flex-shrink:0';
    banner.innerHTML = `
      <div style="font-size:16px;flex-shrink:0">⚡</div>
      <div style="flex:1">
        <div style="font-size:10px;font-weight:700;color:#F5A800">${daysLeft} jour${daysLeft!==1?'s':''} restant${daysLeft!==1?'s':''}</div>
        <div style="font-size:9px;color:#888;margin-top:1px">Ton accès expire bientôt</div>
      </div>
      <button onclick="showRenewalModal()" style="background:#F5A800;color:#000;border:none;border-radius:8px;padding:7px 14px;font-size:11px;font-weight:800;cursor:pointer;font-family:inherit;flex-shrink:0">Renouveler</button>`;
