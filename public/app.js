...
async function fetchStatsFromServer() {
  try {
    ...
    if (sigs === 0 && wr === 0) return; // server returned empty — don't overwrite

    // Update stat bar immediately
    const wrEl   = document.getElementById('stat-winrate');
    const sigsEl = document.getElementById('stat-signals');
    if (wrEl)   wrEl.textContent   = sigs > 0 ? wr + '%' : wrEl.textContent;
    if (sigsEl) sigsEl.textContent = sigs > 0 ? sigs     : sigsEl.textContent;
