...
function applyState(d) {
  ...
  const activeTrade = d.activeTrade;
  const signals     = d.signals || [];
  const broadcast   = d.broadcast;
  const lastTrade   = d.lastTrade;
  ...
  if (broadcast && broadcast.message) {
    ...
    if (saved.msg !== broadcast.message) {
      localStorage.setItem('14t_broadcast', JSON.stringify({
        msg: broadcast.message, ts: broadcast.ts || Date.now(), dismissed: false
      }));
      const card  = document.getElementById('bc-card');
      const msgEl = document.getElementById('bc-card-msg');
      if (card && msgEl) { msgEl.textContent = broadcast.message; card.style.display = 'block'; }
    } else if (!saved.dismissed) {
      const card  = document.getElementById('bc-card');
      const msgEl = document.getElementById('bc-card-msg');
      if (card && msgEl && card.style.display === 'none') {
        msgEl.textContent = broadcast.message; card.style.display = 'block';
      }
    }
  }
  ...
  const cdShowing = cdOverlay && cdOverlay.style.display !== 'none';
  if (activeTrade && activeTrade.status === 'open') {
    // Trade is live — dismiss countdown and show trade card
    dismissCountdown();
    if (activeTrade.id !== lastTradeId) lastTradeId = activeTrade.id;
    updateLiveTrade(activeTrade);
  } else if (activeTrade && activeTrade.status === 'confirmed' && !cdShowing) {
    // Confirmed but no countdown showing — show countdown if prepSignal exists
    if (!prepSignal) updateLiveTrade(activeTrade);
    // If prepSignal exists countdown was already shown above
  } else if (!activeTrade) {
    lastTradeId = null;
...
async function fetchLivePrices() {
  ...
  try {
    ...
    const gNas = document.getElementById('grid-nas-price');
    const gUs30 = document.getElementById('grid-us30-price');
    if (gNas  && d.nas100) gNas.textContent  = '$' + Math.round(d.nas100).toLocaleString();
    if (gUs30 && d.us30)   gUs30.textContent = '$' + Math.round(d.us30).toLocaleString();
  } catch(e) {}
