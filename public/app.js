...
function applyState({ activeTrade, signals, prepSignal, broadcast }) {
...
function updateLiveTrade(trade) {
  ...
  const liveBlock = document.getElementById('live-trade') || document.getElementById('live-block');
  ...
  if (!trade || trade.status === 'closed') {
    ...
    if (scanBlock) scanBlock.style.display = 'flex';
    if (fomoCard)  fomoCard.style.display  = 'none';
    clearInterval(pnlInterval); pnlInterval = null;
    return;
  }

  if (liveBlock) liveBlock.style.display = 'flex';
  ...
  const assetName = (trade.asset || 'GOLD').toUpperCase();

  // Set asset label — support both old and new IDs
  const assetEl = document.getElementById('lt-asset') || document.getElementById('live-asset');
  if (assetEl) assetEl.textContent = assetName;

  // Set direction — support both old and new IDs
  const dirEl = document.getElementById('lt-direction') || document.getElementById('live-dir');
  if (dirEl) { dirEl.textContent = trade.direction; dirEl.style.color = dirColor; }

  // Set SL — support both old and new IDs
  const slEl = document.getElementById('lt-sl') || document.getElementById('sl-price');
  if (slEl) slEl.textContent = trade.sl ? formatPrice(trade.sl) : '--';

  // Mark TPs — support both old and new IDs
  if (trade.tp1_hit) {
    const tp1 = document.getElementById('lt-tp1') || document.getElementById('tp1');
    const tp1s = document.getElementById('lt-tp1-s');
    if (tp1) tp1.classList.add('hit');
    if (tp1s) tp1s.textContent = '✓';
    markTPHit(1, true);
  }
  if (trade.tp2_hit) {
    const tp2 = document.getElementById('lt-tp2') || document.getElementById('tp2');
    const tp2s = document.getElementById('lt-tp2-s');
    if (tp2) tp2.classList.add('hit');
    if (tp2s) tp2s.textContent = '✓';
    markTPHit(2, true);
  }
  if (trade.tp3_hit) {
    const tp3 = document.getElementById('lt-tp3') || document.getElementById('tp3');
    const tp3s = document.getElementById('lt-tp3-s');
    if (tp3) tp3.classList.add('hit');
    if (tp3s) tp3s.textContent = '✓';
    markTPHit(3, true);
  }
  if (trade.sl_hit) markSLHit(true);

  // BE banner
  const beBanner = document.getElementById('lt-be-banner');
  if (beBanner) beBanner.style.display = trade.be_hit ? 'block' : 'none';
}
