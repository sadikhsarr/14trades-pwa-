...
function showBroadcastMessage(msg) {
  ...
  localStorage.setItem('14t_broadcast', JSON.stringify({ msg, ts: String(ts), dismissed: false }));
  ...
  setTimeout(() => renderBroadcastBanner(msg, ts), 400);
}
...
function applyState(d) {
  ...
  const bcTime  = document.getElementById('bc-card-time');
  if (broadcast && broadcast.message) {
    ...
    if (bcCard && bcMsg) {
      ...
      if (bcTime) {
        const mins = Math.floor((Date.now() - (broadcast.ts||Date.now())) / 60000);
        bcTime.textContent = mins < 1 ? 'maintenant' : 'il y a ' + mins + ' min';
      }
      if (isNew) {
        // New broadcast — always show, reset dismissed state
        localStorage.setItem('14t_broadcast', JSON.stringify({ts:bcTs, msg:broadcast.message, dismissed:false}));
