...
async function bootApp() {
  ...
  loadReferralBanner();
  // Retry after 1s and 3s in case access code loads late
...
function getSigCardHTML(trade) {
  ...
  return `<div class="sig-card ${isBuy?'buy':'sell'}" id="card-${trade.id}">
    ...
    ${trade.entry ? `<div style="display:flex;align-items:center;justify-content:space-between;padding:6px 10px;background:#0A0A0A;border-radius:8px;border:1px solid #2A2A2A;margin-bottom:2px">
      <div style="font-size:9px;font-weight:700;color:#636366;letter-spacing:1px;text-transform:uppercase">Entry</div>
      <div style="font-family:'Space Mono',monospace;font-size:12px;font-weight:700;color:#F5F5F0">${trade.entry}</div>
    </div>` : ''}
