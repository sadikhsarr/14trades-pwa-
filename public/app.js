...
function checkAccess() {
  ...
  if (saved && expiry > now) {
    ...
    // Has code but expiry missing/past — always revalidate with server
    // Never just delete — server is the source of truth
    revalidateCode(saved);
  } else {
    // No code at all — show public screen
    showPublicScreen();
  }
}
...
async function revalidateCode(code) {
  try {
    ...
    const d = await r.json();
    if (d.valid) {
      // Still valid — restore expiry (minimum 30 days from now)
    ...
    // Network error — if code exists assume valid and boot
    hidePublicScreen();
    bootApp();
  }
}
...
document.addEventListener('DOMContentLoaded', async () => {
  checkAccess();
  // Safety fallback — only show public screen if NO valid code exists at all
  setTimeout(function() {
    ...
    // Only show public screen if no valid code — never override an active session
