...
function showShareCardFromHistory(sig) {
  showToast('DEBUG: clic détecté, sig=' + JSON.stringify(sig).slice(0,80), 'info');
  if (!sig) { showToast('DEBUG: sig est vide/undefined!', 'warn'); return; }
  ...
  const adapted = {
    ...
    // No stored price levels for historical entries — card shows "—"
    // for these, same graceful fallback already built into the card.
    entry: null, sl: null, tp1: null, tp2: null, tp3: null,
    _historicalDate: sig.timestamp,
  };
  try {
    populateShareCard(adapted);
    showToast('DEBUG: populateShareCard terminé sans erreur', 'buy');
  } catch(e) {
...
function populateShareCard(trade) {
  ...
  const TP_PIPS = {1:120, 2:200, 3:350};
  const pips   = isSL ? -120 : isBE ? 0 : TP_PIPS[tp] || 0;
