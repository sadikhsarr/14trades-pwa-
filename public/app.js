...
function showShareCard(tradeId) {
  ...
  setColor('sc-dir-lbl', isBuy?'#1A6B3C':'#8B2020');
  set('sc-session', sess);

  // Big pips number
  set('sc-pips', pipsStr);
  setColor('sc-pips', pips===0?'#555':resultColor);
...
// ── Auto-request push on first login ─────────────────────────
async function showNotifPrompt() {
  ...
  // Ask after 3s so user is settled in the app
  setTimeout(async () => {
    try { await requestPush(); }
    catch(e) {}
  }, 3000);
}
