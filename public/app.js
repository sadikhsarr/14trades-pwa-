...
async function fetchLivePriceForCountdown(asset) {
  try {
    ...
    } else if (asset === 'NAS100' || asset === 'NASDAQ') {
      const r = await fetch('https://query1.finance.yahoo.com/v8/finance/spark?symbols=NQ%3DF&range=1d&interval=5m');
      const d = await r.json();
      const p = d.spark?.result?.[0]?.response?.[0]?.meta?.regularMarketPrice;
      if (p) priceEl.textContent = '$' + Math.round(p).toLocaleString();

    } else if (asset === 'US30') {
      const r = await fetch('https://query1.finance.yahoo.com/v8/finance/spark?symbols=YM%3DF&range=1d&interval=5m');
      const d = await r.json();
      const p = d.spark?.result?.[0]?.response?.[0]?.meta?.regularMarketPrice;
      if (p) priceEl.textContent = '$' + Math.round(p).toLocaleString();
...
async function fetchLivePrices() {
  ...
  // Nasdaq + US30
  try {
    const r = await fetch('https://query1.finance.yahoo.com/v8/finance/spark?symbols=NQ%3DF%2CYM%3DF&range=1d&interval=5m');
    const d = await r.json();
    const results = d.spark?.result || [];
    const nas  = results[0]?.response?.[0]?.meta?.regularMarketPrice;
    const us30 = results[1]?.response?.[0]?.meta?.regularMarketPrice;
    const nasEl  = document.getElementById('mood-nas-price');
    const us30El = document.getElementById('mood-us30-price');
    if (nasEl  && nas)  nasEl.textContent  = '$' + Math.round(nas).toLocaleString();
    if (us30El && us30) us30El.textContent = '$' + Math.round(us30).toLocaleString();
  } catch(e) {}
  // Session
  try {
    const nyH = parseInt(new Date().toLocaleTimeString('en-US',{timeZone:'America/New_York',hour12:false,hour:'2-digit'}));
    let session = 'Closed';
    ...
    if (pel) pel.textContent = 'NY ' + new Date().toLocaleTimeString('en-US',{timeZone:'America/New_York',hour12:false,hour:'2-digit',minute:'2-digit'});
  } catch(e) {}
}
