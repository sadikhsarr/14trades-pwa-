...
async function fetchLivePrices() {
  // Gold + Silver — try goldprice.dev first (real exchange sourcing, bid/ask,
  // no API key needed), fall back to metals.live if anything's unexpected.
  let goldPrice = null, silverPrice = null;
  try {
    const r = await fetch('https://api.goldprice.dev/v1/prices?symbol=XAU-USD-SPOT');
    const d = await r.json();
    // Defensive parsing — try the most likely field names in order
    goldPrice = d?.price ?? d?.rate ?? d?.spot ?? d?.data?.price ?? d?.prices?.[0]?.price ?? null;
  ...
  if (goldPrice != null) {
    ...
    // goldprice.dev unavailable/unexpected shape — use the original source
    try {
      const r = await fetch('https://api.metals.live/v1/spot/gold');
      const d = await r.json();
      const p = d[0]?.price;
