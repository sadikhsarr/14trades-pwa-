...
function checkAccess() {
  ...
  if (saved && expiry > now) {
    ...
    if (saved) {
      // Had an account before — show expired screen
      showExpiredScreen(wasTrial);
    } else {
      // Never had account — show public screen
      showPublicScreen();
    }
  }
}
  ...
  try {
    ...
    feed.innerHTML = items.map(function(item) {
      if (item.type === 'broadcast') {
        ...
        return '<div style="background:#0D0D0D;border:1px solid ' + (b.pinned ? '#FFD70025' : '#1A1A1A') + ';border-radius:14px;padding:16px;margin-bottom:10px">' +
          '<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">' +
            '<span style="font-size:16px">' + icon + '</span>' +
            '<div style="flex:1;font-size:13px;font-weight:600;color:#F0F0F0">' + b.title + '</div>' +
            (b.pinned ? '<span style="font-size:9px;padding:2px 8px;border-radius:20px;background:#FFD70015;color:#FFD700;font-weight:700;border:1px solid #FFD70025">PIN</span>' : '') +
          '</div>' +
          '<div style="font-size:12px;color:#666;line-height:1.6;margin-bottom:8px;white-space:pre-wrap">' + b.message + '</div>' +
          '<div style="font-size:10px;color:#2A2A2A">' + date + '</div>' +
          '</div>';
      } else {
        var s = item.data;
        var dir = s.direction || s.dir || 'BUY';
        var asset = s.asset || 'GOLD';
        var color = dir === 'BUY' ? '#00C87A' : '#FF3B4E';
        var date = item.date.toLocaleDateString('fr-FR', { day:'2-digit', month:'short', hour:'2-digit', minute:'2-digit' });
        return '<div style="background:#0D0D0D;border:1px solid ' + color + '20;border-radius:14px;padding:16px;margin-bottom:10px">' +
          '<div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">' +
            '<span style="font-size:14px">⚡</span>' +
            '<div style="font-size:11px;font-weight:700;color:' + color + ';letter-spacing:1px;text-transform:uppercase">Signal · ' + asset + '</div>' +
            '<div style="margin-left:auto;font-family:Space Mono,monospace;font-size:11px;font-weight:700;color:' + color + '">' + dir + '</div>' +
          '</div>' +
          '<div style="font-size:11px;color:#444">Entrée · SL · TP1 · TP2 · TP3 — membres uniquement 🔒</div>' +
          '<div style="font-size:10px;color:#2A2A2A;margin-top:6px">' + date + '</div>' +
          '</div>';
      }
    }).join('');
...
function showPublicScreen() {
  ...
  if (ps) { ps.style.display = 'block'; loadPublicBroadcasts(); }
}
