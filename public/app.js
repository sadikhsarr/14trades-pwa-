...
function renderBroadcastBanner(msg) {
  ...
  banner.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:800;background:#0D1520;border-bottom:1px solid #4A9EFF30;padding:12px 18px;display:flex;align-items:center;gap:10px;animation:slide-down .3s ease';
  banner.innerHTML = `
    <div style="font-size:16px;flex-shrink:0">📢</div>
    <div style="flex:1">
      <div style="font-size:8px;font-weight:700;letter-spacing:1.5px;color:#4A9EFF;text-transform:uppercase;margin-bottom:3px">From Sadikh · 14 Trades</div>
      <div style="font-size:12px;color:#F0F0F0;line-height:1.5">${msg}</div>
    </div>
    <div onclick="dismissBroadcast()" style="color:#555;font-size:18px;cursor:pointer;padding:4px;flex-shrink:0">✕</div>`;
...
function checkSavedBroadcast() {
  try {
    ...
    if (age < 24 * 60 * 60 * 1000) {
      setTimeout(() => renderBroadcastBanner(saved.msg), 1500);
    } else {
