...
function checkAccess() {
  ...
  if (saved && expiry > now) {
    ...
    // Show inline login — no redirect needed
    showLoginOverlay();
  }
}
...
function showLastTradeResult(trade) {
  ...
  card.innerHTML = `
    <div style="font-size:8px;font-weight:700;letter-spacing:1.5px;color:${color};text-transform:uppercase;margin-bottom:6px">Last Signal · ${minsAgo} min ago</div>
...
async function poll() {
  try {
    const r = await fetch(SERVER + '/api/state?t=' + Date.now());
    const d = await r.json();
    lastState = d;

    // SHOW TRADE immediately — no complex checks
    ...
    if (trade && (trade.status === 'open' || trade.status === 'confirmed')) {
      // Show trade card
      ...
      const assetPairs = {GOLD:'XAU/USD',SILVER:'XAG/USD',BTC:'BTC/USD',NAS100:'NAS100',US30:'DOW JONES'};
      if (a) a.textContent = trade.asset + ' · ' + (assetPairs[trade.asset] || trade.asset);
      if (dir) { dir.textContent = trade.direction; dir.style.color = trade.direction === 'BUY' ? '#30D158' : '#FF453A'; }
      ...
      if (be) be.style.display = trade.be_hit ? 'block' : 'none';

    } else {
      // No trade — show scanning
    ...
    if (d.broadcast && d.broadcast.message) {
      const saved = JSON.parse(localStorage.getItem('14t_broadcast') || '{}');
      if (saved.msg !== d.broadcast.message) {
        localStorage.setItem('14t_broadcast', JSON.stringify({msg: d.broadcast.message, ts: d.broadcast.ts || Date.now(), dismissed: false}));
        const card = document.getElementById('bc-card');
        const msgEl = document.getElementById('bc-card-msg');
        if (card && msgEl) { msgEl.textContent = d.broadcast.message; card.style.display = 'block'; }
      } else if (!saved.dismissed) {
        const card = document.getElementById('bc-card');
        const msgEl = document.getElementById('bc-card-msg');
        if (card && msgEl && card.style.display === 'none') { msgEl.textContent = d.broadcast.message; card.style.display = 'block'; }
      }
    }

    // Render history
    if (d.signals) renderHistory(d.signals);

  } catch(e) { console.error('[POLL ERROR]', e); }
}

// Keep old fetchState as alias
function fetchState() { poll(); }
function applyState(d) { /* handled in poll() */ }

function applyState({ activeTrade, signals, prepSignal, broadcast, lastTrade }) {
  // Show broadcast card if server has one — only once, not every poll
  if (broadcast && broadcast.message) {
    const saved = JSON.parse(localStorage.getItem('14t_broadcast') || '{}');
    if (saved.msg !== broadcast.message) {
      // New message — save and show
      localStorage.setItem('14t_broadcast', JSON.stringify({
        msg: broadcast.message,
        ts: broadcast.ts || Date.now(),
        dismissed: false
      }));
      renderBroadcastBanner(broadcast.message, broadcast.ts || Date.now());
    } else if (!saved.dismissed) {
      // Same message, not dismissed — show card only (no banner)
      const card  = document.getElementById('bc-card');
      const msgEl = document.getElementById('bc-card-msg');
      if (card && msgEl && card.style.display === 'none') {
  ...
  if (activeTrade && (activeTrade.status === 'open' || activeTrade.status === 'confirmed')) {
    if (activeTrade.status === 'open') dismissCountdown();
    // If confirmed but no countdown showing — show trade card directly
    const cdOverlay = document.getElementById('signal-countdown');
    const cdVisible = cdOverlay && cdOverlay.style.display !== 'none';
    if (activeTrade.status === 'confirmed' && !cdVisible) {
      // Show trade card immediately — member missed the countdown push
      updateLiveTrade(activeTrade);
    }
    if (activeTrade.id !== lastTradeId) {
      if (lastTradeId !== null && activeTrade.status === 'open') showSignalFlash(activeTrade);
    ...
    // Show last trade result if closed recently
    if (lastTrade) showLastTradeResult(lastTrade);
  }
...
function updateLiveTrade(trade) {
  ...
  // Asset
  const a = document.getElementById('lt-asset');
  if (a) a.textContent = (trade.asset || 'GOLD').toUpperCase() + ' · ' + (trade.asset === 'BTC' ? 'BTC/USD' : trade.asset === 'SILVER' ? 'XAG/USD' : trade.asset === 'NAS100' ? 'NAS100' : trade.asset === 'US30' ? 'DOW JONES' : 'XAU/USD');

  // Direction
  const d = document.getElementById('lt-direction');
  if (d) { d.textContent = trade.direction || ''; d.style.color = isBuy ? '#30D158' : '#FF453A'; }
  ...
  // SL hit
  if (trade.sl_hit) {
    const slRow = document.getElementById('lt-sl-row') || document.querySelector('.lt-sl-row');
    if (slRow) slRow.style.borderColor = '#FF453A50';
  }
}
