...
// ── Securely fetch the real prep-signal data (server enforces membership) ──
async function showPrepCountdownSecure(fallbackId, fallbackSecs) {
  try {
    const _code = localStorage.getItem('14t_access') || '';
    const r = await fetch(SERVER + '/api/state?t=' + Date.now() + '&code=' + encodeURIComponent(_code), { cache: 'no-store' });
    const d = await r.json();
    if (d.prepSignal && d.prepSignal.id) {
      showSignalCountdown(d.prepSignal);
      return;
    }
  } catch(e) {}
  // Fallback — show generic countdown with no signal details revealed
  showSignalCountdown({ id: fallbackId, asset: null, direction: null, secondsUntilSignal: fallbackSecs || 60 });
}
...
function checkPrepFromURL() {
  try {
    ...
    if (p.type === 'prep') {
      ...
      const showCD = () => {
        speakAlert('prep');
        showPrepCountdownSecure(p.id, secs);
      };
...
// ── Missed Signals Screen — for expired former members ──────────────
async function showMissedSignalsScreen() {
  // Create overlay
  const overlay = document.createElement('div');
  overlay.id = 'missed-screen';
  overlay.style.cssText = 'position:fixed;inset:0;z-index:900;background:#080808;overflow-y:auto;font-family:Inter,system-ui,sans-serif;-webkit-font-smoothing:antialiased';

  overlay.innerHTML = `
    <div style="max-width:420px;margin:0 auto;padding:32px 20px 100px">

      <!-- Header -->
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:24px">
        <span style="font-family:monospace;font-size:24px;font-weight:700;color:#C9A84C;letter-spacing:-1px">14</span>
        <span style="font-size:18px">⚡</span>
        <div style="font-size:7px;font-weight:700;letter-spacing:4px;color:#F0F2F8">TRADES ALERT</div>
      </div>

      <!-- Live signal banner — only shown if a real signal is active right now -->
      <div id="missed-live-banner" style="display:none;margin-bottom:18px">
        <style>
          @keyframes msExPulse{0%,100%{opacity:.4;transform:scale(1)}50%{opacity:1;transform:scale(1.4)}}
          @keyframes msExSpin{to{transform:rotate(360deg)}}
          .ms-ex-border{position:relative;border-radius:16px;padding:1px;overflow:hidden;background:#0A0A0A}
          .ms-ex-border::before{content:'';position:absolute;inset:-70%;background:conic-gradient(from 0deg,#00C87A,#00000000 25%,#00000000 75%,#00C87A);animation:msExSpin 5s linear infinite}
          .ms-ex-inner{position:relative;background:radial-gradient(ellipse at center,#0C1810,#060504);border-radius:15px}
        </style>
        <div class="ms-ex-border">
          <div class="ms-ex-inner" style="padding:14px">
            <div style="display:flex;align-items:center;gap:6px;margin-bottom:8px">
              <span style="position:relative;width:7px;height:7px"><span style="position:absolute;inset:0;border-radius:50%;background:#00C87A;animation:msExPulse 1.5s infinite"></span><span style="position:absolute;inset:0;border-radius:50%;background:#00C87A"></span></span>
              <div style="font-size:9px;font-weight:800;color:#00E68A;letter-spacing:1px">SIGNAL ACTIF EN CE MOMENT</div>
            </div>
            <div id="missed-live-text" style="font-size:12px;color:#F0F2F8;font-weight:600">—</div>
            <div style="font-size:9px;color:#666;margin-top:2px">Tu ne le vois pas. Renouvelle pour débloquer.</div>
          </div>
        </div>
      </div>

      <!-- Broadcast from Sadikh — e.g. "prochain signal probable bientôt" -->
      <div id="missed-broadcast-banner" style="display:none;margin-bottom:18px;background:linear-gradient(145deg,#141000,#0D0A00);border:1px solid #C9A84C30;border-radius:14px;padding:14px">
        <div id="missed-bc-label" style="font-size:9px;font-weight:800;color:#C9A84C;letter-spacing:1px;margin-bottom:5px">📢 MESSAGE · SADIKH</div>
        <div id="missed-bc-text" style="font-size:12px;color:#F0F2F8;line-height:1.5">—</div>
      </div>

      <!-- Watch Signal countdown — real "surveillance active" from admin -->
      <div id="missed-watch-banner" style="display:none;margin-bottom:18px;background:linear-gradient(145deg,#1A1400,#0D0A00);border:1px solid #FFD70030;border-radius:14px;padding:14px;text-align:center">
        <div style="font-size:9px;font-weight:800;color:#FFD700;letter-spacing:1px;margin-bottom:6px">👀 SURVEILLANCE ACTIVE · <span id="missed-watch-asset">—</span></div>
        <div id="missed-watch-countdown" style="font-family:monospace;font-size:24px;font-weight:700;color:#FFD700;margin-bottom:6px">--:--:--</div>
        <div style="font-size:9px;color:#666">Sadikh surveille ce setup — le signal peut arriver à tout moment</div>
      </div>

      <!-- Opportunity-framed message -->
      <div style="margin-bottom:20px">
        <div style="font-size:20px;font-weight:800;color:#F0F2F8;line-height:1.3;margin-bottom:8px">Voici ce qui t'attend<br><span style="color:#C9A84C">dès que tu reviens.</span></div>
        <div style="font-size:12px;color:#666;line-height:1.6">Sadikh continue d'analyser Gold chaque jour. Ton accès peut être actif en 2 minutes.</div>
      </div>

      <!-- Real stats — win rate + missed pips + real member count -->
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;margin-bottom:14px">
        <div style="background:#0D0D0D;border:1px solid #C9A84C25;border-radius:10px;padding:10px 6px;text-align:center">
          <div style="font-size:7px;color:#555">TAUX</div>
          <div id="missed-winrate" style="font-family:monospace;font-size:16px;font-weight:800;color:#C9A84C;margin-top:2px">—</div>
        </div>
        <div style="background:#0D0D0D;border:1px solid #00C87A25;border-radius:10px;padding:10px 6px;text-align:center">
          <div style="font-size:7px;color:#555">MANQUÉ</div>
          <div id="missed-pips" style="font-family:monospace;font-size:16px;font-weight:800;color:#00C87A;margin-top:2px">—</div>
        </div>
        <div style="background:#0D0D0D;border:1px solid #4A9EFF25;border-radius:10px;padding:10px 6px;text-align:center">
          <div style="font-size:7px;color:#555">MEMBRES</div>
          <div id="missed-members" style="font-family:monospace;font-size:16px;font-weight:800;color:#4A9EFF;margin-top:2px">—</div>
        </div>
      </div>

      <a href="/track-record.html" style="display:flex;align-items:center;justify-content:center;gap:6px;font-size:10px;color:#C9A84C;text-decoration:none;margin-bottom:16px">
        <i class="ti ti-shield-check" style="font-size:12px" aria-hidden="true"></i>
        Vérifie ces chiffres toi-même →
      </a>

      <!-- Reassurance card -->
      <div style="background:linear-gradient(145deg,#141000,#0D0A00);border:1px solid #C9A84C30;border-radius:14px;padding:14px;margin-bottom:20px;text-align:center">
        <div style="font-size:11px;color:#AAA;line-height:1.5">Tu n'as pas besoin d'avoir raison souvent —</div>
        <div style="font-size:11px;color:#AAA;line-height:1.5"><span style="color:#FFD700;font-weight:800">2 wins sur 5</span> suffisent pour être positif</div>
      </div>

      <!-- Recent signals (supporting detail) -->
      <div id="missed-signals-list" style="margin-bottom:20px">
        <div style="text-align:center;padding:20px;color:#333;font-size:12px">Chargement...</div>
      </div>

      <!-- CTA — Crypto promoted as the single recommended path (genuinely
           fastest, automatic), Wave/Orange tucked into a collapsible for
           anyone who prefers them. Fewer competing choices, less hesitation. -->
      <div style="background:linear-gradient(135deg,#0A1A0A,#0D0D0D);border:1px solid #00C87A40;border-radius:14px;padding:16px;margin-bottom:8px">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:2px">
          <div style="font-size:12px;font-weight:800;color:#00E68A">₿ Crypto — recommandé</div>
          <div style="font-size:8px;background:#00C87A20;color:#00E68A;border-radius:6px;padding:2px 7px;font-weight:700">INSTANTANÉ</div>
        </div>
        <div style="font-size:9px;color:#888;margin-bottom:10px">100 USDT · activation automatique, accès en 2 min</div>
        <button onclick="selectPaymentMissed('crypto')" style="width:100%;background:#00C87A;color:#000;border:none;border-radius:9px;padding:12px;text-align:center;font-size:12px;font-weight:800;cursor:pointer;font-family:inherit">Débloquer maintenant →</button>
      </div>

      <details style="margin-bottom:16px">
        <summary style="font-size:11px;color:#555;cursor:pointer;padding:8px 4px">Autres méthodes (Wave, Orange Money)</summary>
        <div style="display:flex;flex-direction:column;gap:8px;margin-top:10px">
          <button onclick="selectPaymentMissed('wave')" style="display:flex;align-items:center;gap:12px;background:#0A1A2A;border:1px solid #1E90FF25;border-radius:10px;padding:12px 14px;cursor:pointer;font-family:inherit;width:100%">
            <span style="font-size:18px">🌊</span>
            <div style="text-align:left"><div style="font-size:12px;font-weight:700;color:#F0F2F8">Wave</div><div style="font-size:9px;color:#555">57 000 CFA</div></div>
            <div style="margin-left:auto;font-size:11px;color:#1E90FF">→</div>
          </button>
          <button onclick="selectPaymentMissed('orange')" style="display:flex;align-items:center;gap:12px;background:#1A0F00;border:1px solid #FF6B0025;border-radius:10px;padding:12px 14px;cursor:pointer;font-family:inherit;width:100%">
            <span style="font-size:18px">🟠</span>
            <div style="text-align:left"><div style="font-size:12px;font-weight:700;color:#F0F2F8">Orange Money</div><div style="font-size:9px;color:#555">57 000 CFA</div></div>
            <div style="margin-left:auto;font-size:11px;color:#FF6B00">→</div>
          </button>
        </div>
      </details>
      ...
      <div style="text-align:center">
        <button onclick="showLogin('code')" style="background:none;border:none;color:#444;font-size:12px;cursor:pointer;font-family:inherit;text-decoration:underline">J'ai déjà un code d'accès</button>
      </div>
  ...
  // Real win rate — same honest track-record source used elsewhere
  fetch(SERVER + '/api/track-record?t=' + Date.now(), {cache:'no-store'})
    .then(r => r.json())
    .then(d => {
      const el = document.getElementById('missed-winrate');
      if (el && d.ok && d.stats) el.textContent = (d.stats.winRate ?? 0) + '%';
    })
    .catch(() => {});

  // Real member count — same source as /api/stats elsewhere
  fetch(SERVER + '/api/stats?t=' + Date.now(), {cache:'no-store'})
    .then(r => r.json())
    .then(d => {
      const el = document.getElementById('missed-members');
      if (el && d.ok && typeof d.members === 'number' && d.members > 0) el.textContent = d.members;
    })
    .catch(() => {});

  // Live signal happening right now — non-member request gives real
  // pipsInFavor (magnitude only, no direction) via /api/state's FOMO logic
  fetch(SERVER + '/api/state?t=' + Date.now(), {cache:'no-store'})
    .then(r => r.json())
    .then(d => {
      const trade = (d.activeTrades || [])[0];
      if (trade) {
        const banner = document.getElementById('missed-live-banner');
        const textEl = document.getElementById('missed-live-text');
        if (banner && textEl) {
          const asset = (trade.asset || 'Gold').toUpperCase();
          textEl.textContent = asset + ' · en cours' + (trade.pipsInFavor != null ? ` — +${trade.pipsInFavor} pips pour les membres` : '');
          banner.style.display = 'block';
        }
      }
      // Broadcast — e.g. "prochain signal probable bientôt", same admin
      // tool already used to message logged-in members, now also reaching
      // people on this screen.
      if (d.broadcast && d.broadcast.message) {
        const bcBanner = document.getElementById('missed-broadcast-banner');
        const bcText   = document.getElementById('missed-bc-text');
        const bcLabel  = document.getElementById('missed-bc-label');
        if (bcBanner && bcText) {
          bcText.textContent = d.broadcast.message;
          if (bcLabel) bcLabel.textContent = d.broadcast.type === 'reflection' ? '📓 RÉFLEXION · SADIKH' : '📢 MESSAGE · SADIKH';
          bcBanner.style.display = 'block';
        }
      }
      // Watch Signal countdown — same "surveillance active" tool already
      // shown to logged-in members, now also reaching people on this screen.
      if (d.watchSignal && d.watchSignal.expiresAt > Date.now()) {
        const wBanner = document.getElementById('missed-watch-banner');
        const wAsset  = document.getElementById('missed-watch-asset');
        const wCount  = document.getElementById('missed-watch-countdown');
        if (wBanner && wAsset && wCount) {
          wAsset.textContent = d.watchSignal.asset || '—';
          wBanner.style.display = 'block';
          const expiresAt = d.watchSignal.expiresAt;
          const tick = () => {
            const msLeft = expiresAt - Date.now();
            if (msLeft <= 0) { wCount.textContent = '00:00:00'; clearInterval(missedWatchTicker); return; }
            const h = Math.floor(msLeft / 3600000);
            const m = Math.floor((msLeft % 3600000) / 60000);
            const s = Math.floor((msLeft % 60000) / 1000);
            wCount.textContent = `${h.toString().padStart(2,'0')}:${m.toString().padStart(2,'0')}:${s.toString().padStart(2,'0')}`;
          };
          if (missedWatchTicker) clearInterval(missedWatchTicker);
          missedWatchTicker = setInterval(tick, 1000);
          tick();
        }
      }
    })
  ...
  // Load missed signals from server
  try {
    const r = await fetch(SERVER + '/api/state?t=' + Date.now(), {cache:'no-store'});
    const d = await r.json();
    const signals = (d.signals||[]).filter(s => s.type === 'tp_hit' || s.type === 'sl_hit');
    const list = document.getElementById('missed-signals-list');
    const pipsEl = document.getElementById('missed-pips');

    if (signals.length > 0) {
      const TP_PIPS = {1:120, 2:200, 3:350};
      let totalPips = 0;
      signals.forEach(s => {
        if (s.type === 'tp_hit') totalPips += TP_PIPS[s.level]||120;
        else totalPips -= 120;
      });

      list.innerHTML = '<div style="font-size:9px;color:#333;letter-spacing:1px;margin-bottom:8px">DERNIERS SIGNAUX</div>' + signals.slice(0,4).map(s => {
        const isWin = s.type === 'tp_hit';
        const pips = isWin ? (TP_PIPS[s.level]||120) : -120;
        const color = isWin ? '#00C87A' : '#FF3B4E';
        const date = new Date(s.timestamp).toLocaleDateString('fr-FR',{day:'2-digit',month:'short'});
        const label = isWin ? 'TP'+(s.level||1)+' ✓' : 'SL ✗';
        return `<div style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;background:#0D0D0D;border:1px solid #1A1A1A;border-radius:10px;margin-bottom:7px">
          <div style="display:flex;align-items:center;gap:10px">
            <div style="width:3px;height:36px;background:${color};border-radius:2px;flex-shrink:0"></div>
            <div>
              <div style="font-size:12px;font-weight:600;color:#F0F2F8">${s.asset||'Gold'} ${s.direction||''} · ${label}</div>
              <div style="font-size:9px;color:#2A2A2A;margin-top:2px">${date}</div>
            </div>
          </div>
          <div style="font-family:monospace;font-size:16px;font-weight:700;color:${color}">${pips>0?'+':''}${pips}p</div>
        </div>`;
      }).join('');

      if (pipsEl) pipsEl.textContent = (totalPips >= 0 ? '+' : '') + totalPips;
    } else {
      list.innerHTML = '<div style="text-align:center;padding:16px;color:#333;font-size:12px">Aucun signal récent — analyse en cours.</div>';
      if (pipsEl) pipsEl.textContent = '—';
    }
  } catch(e) {
    document.getElementById('missed-signals-list').innerHTML = '<div style="text-align:center;padding:16px;color:#333;font-size:12px">Impossible de charger les signaux.</div>';
  }
}

function selectPaymentMissed(method) {
  const PAYMENT_INFO = {
    wave: {
      number: '+221 78 825 50 99',
      msg: encodeURIComponent('Bonjour Sadikh, je veux renouveler mon accès 30 jours via Wave. 57 000 CFA envoyés.'),
    },
    orange: {
      number: '+221 78 825 50 99',
      msg: encodeURIComponent('Bonjour Sadikh, je veux renouveler mon accès 30 jours via Orange Money. 57 000 CFA envoyés.'),
    }
  };
  if (method === 'crypto') {
    // Remove missed screen, show login to get code, then crypto flow
    const ms = document.getElementById('missed-screen');
    if (ms) ms.remove();
    showLogin('code');
    return;
  }
  const info = PAYMENT_INFO[method];
  window.open('https://wa.me/221788255099?text=' + info.msg, '_blank');
}
...
function checkAccess() {
  ...
  if (code && expiry > now) {
    ...
    // Expired or no code
    var wasEverMember = localStorage.getItem('14t_join');
    var hadCode = code && expRaw; // had a code that expired
    if (wasEverMember && hadCode) {
      // Former member — show missed signals screen instead of public page
      localStorage.removeItem('14t_access');
      localStorage.removeItem('14t_access_exp');
      localStorage.removeItem('14t_just_logged_in');
      showMissedSignalsScreen();
    } else {
      // Never was a member — show public screen
...
async function bootApp() {
  ...
  // Show any broadcast saved from notification tap (before app was open)
  try {
    const _bc = JSON.parse(localStorage.getItem('14t_broadcast') || '{}');
    if (_bc.msg && !_bc.dismissed) renderBroadcastBanner(_bc.msg, parseInt(_bc.ts)||Date.now());
  } catch(e) {}
  try { showDailyRecap(); }     catch(e) {}
  try { resetTodayPnLIfNewDay(); } catch(e) {}
  try { startLivePrices(); }    catch(e) {}
  try { loadWeeklyRecap(); }    catch(e) {}
  ...
  if (savedCode) {
    ...
    // Safety net — some devices take longer than 30s for OneSignal's SDK
    // to resolve a subscription ID. Keep trying periodically as long as
    // we still don't have one saved locally.
    setInterval(() => {
      if (!localStorage.getItem('14t_player_id')) savePlayerIdToServer(savedCode);
    }, 2 * 60 * 1000);
  }
...
navigator.serviceWorker && navigator.serviceWorker.addEventListener('message', event => {
  ...
  if (type === 'NOTIFICATION_CLICKED') {
    ...
    if (p.type === 'prep') {
      ...
      if (prepId !== lastPrepId) {
        lastPrepId = prepId;
        speakAlert('prep');
        showPrepCountdownSecure(prepId, p.secondsUntilSignal || 60);
      }
    } else if (p.type === 'broadcast') {
      const msg = p.message || p.body || p.contents || '';
      if (msg) showBroadcastMessage(msg, p.bcType||'', p.bcLink||'');
  ...
  if (payload.type === 'renewal_confirmed') {
    // Update local expiry
    if (payload.expiresAt) {
      localStorage.setItem('14t_access_exp', String(payload.expiresAt));
    }
    // Remove renewal banner
    const rb = document.getElementById('renewal-banner-top');
    if (rb) rb.remove();
    const rm = document.getElementById('renewal-modal');
    if (rm) rm.remove();
    // Show success
    const toast = document.createElement('div');
    toast.style.cssText = 'position:fixed;top:20px;left:50%;transform:translateX(-50%);background:#00C87A;color:#000;border-radius:10px;padding:12px 20px;font-size:13px;font-weight:700;z-index:2000;animation:slide-down .3s ease';
    toast.textContent = '✅ Accès renouvelé — 30 jours !';
  ...
  if (payload.type === 'broadcast') {
    ...
    showBroadcastMessage(msg, payload.bcType||'', payload.bcLink||'');
    return;
  }
  if (payload.type === 'prep') {
    ...
    if (prepId !== lastPrepId) {
      lastPrepId = prepId;
      speakAlert('prep');
      showPrepCountdownSecure(prepId, payload.secondsUntilSignal || 60);
    }
...
function showRenewalBanner(daysLeft) {
  // Show sticky banner at top
  const existing = document.getElementById('renewal-banner-top');
  if (!existing) {
    const banner = document.createElement('div');
    banner.id = 'renewal-banner-top';
    banner.style.cssText = 'padding:8px 12px 0;flex-shrink:0';
    banner.innerHTML = `
      <style>
        @keyframes rbPulse{0%,100%{opacity:.4;transform:scale(1)}50%{opacity:1;transform:scale(1.4)}}
        @keyframes rbSpin{to{transform:rotate(360deg)}}
        .rb-border{position:relative;border-radius:16px;padding:1px;overflow:hidden;background:#0A0A0A}
        .rb-border::before{content:'';position:absolute;inset:-70%;background:conic-gradient(from 0deg,#F5A800,#00000000 25%,#00000000 75%,#F5A800);animation:rbSpin 6s linear infinite}
        .rb-inner{position:relative;background:linear-gradient(155deg,#1A1400,#0D0A00);border-radius:15px}
      </style>
      <div class="rb-border">
        <div class="rb-inner" style="padding:14px 16px;display:flex;align-items:center;gap:12px">
          <div style="position:relative;width:36px;height:36px;flex-shrink:0">
            <div style="position:absolute;inset:0;border-radius:50%;background:#F5A80016;border:1px solid #F5A80030;display:flex;align-items:center;justify-content:center">
              <div style="position:relative;width:8px;height:8px">
                <div style="position:absolute;inset:0;border-radius:50%;background:#F5A800;animation:rbPulse 1.4s infinite"></div>
                <div style="position:absolute;inset:0;border-radius:50%;background:#F5A800"></div>
              </div>
            </div>
          </div>
          <div style="flex:1">
            <div style="font-size:12px;font-weight:800;color:#F5A800">${daysLeft} jour${daysLeft!==1?'s':''} restant${daysLeft!==1?'s':''}</div>
            <div style="font-size:9px;color:#888;margin-top:1px">Ton accès expire bientôt</div>
          </div>
          <button onclick="showRenewalModal()" style="background:linear-gradient(135deg,#F5A800,#FFD700);color:#1A1000;border:none;border-radius:9px;padding:9px 16px;font-size:11px;font-weight:800;cursor:pointer;font-family:inherit;flex-shrink:0">Renouveler</button>
        </div>
      </div>`;
    // Insert after header, before scrollable
    const main = document.getElementById('main-app');
    if (main) {
      const scrollable = main.querySelector('[style*="padding:10px 14px"]');
      if (scrollable) main.insertBefore(banner, scrollable);
...
function showRenewalModal() {
  ...
  modal.style.cssText = 'position:fixed;inset:0;z-index:1000;background:rgba(0,0,0,.9);display:flex;flex-direction:column;align-items:center;justify-content:flex-end;padding:0;backdrop-filter:blur(8px)';
  modal.innerHTML = `
    <div style="width:100%;max-width:440px;background:#0D0D0D;border-radius:20px 20px 0 0;border-top:1px solid #1A1A1A;padding:24px 20px 40px">

      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">
        <div>
          <div style="font-size:16px;font-weight:800;color:#F0F2F8">Renouveler l'accès</div>
          <div style="font-size:11px;color:#555;margin-top:2px">30 jours · Signaux illimités</div>
        </div>
        <button onclick="document.getElementById('renewal-modal').remove()" style="background:#1A1A1A;border:none;border-radius:50%;width:32px;height:32px;color:#555;font-size:16px;cursor:pointer">✕</button>
      </div>

      <div style="font-family:monospace;font-size:32px;font-weight:700;color:#C9A84C;text-align:center;margin-bottom:6px">57 000 CFA</div>
      <div style="font-size:10px;color:#333;text-align:center;margin-bottom:20px">/ 30 jours</div>

      <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:20px">

        <!-- Wave -->
        <button onclick="selectPayment('wave','${code}')" style="display:flex;align-items:center;gap:14px;background:#0A1A2A;border:1px solid #1E90FF30;border-radius:12px;padding:14px 16px;cursor:pointer;font-family:inherit;width:100%;text-align:left">
          <div style="width:40px;height:40px;border-radius:10px;background:#1E90FF20;display:flex;align-items:center;justify-content:center;font-size:20px;flex-shrink:0">🌊</div>
          <div style="flex:1">
            <div style="font-size:13px;font-weight:700;color:#F0F2F8">Wave</div>
            <div style="font-size:10px;color:#555;margin-top:2px">Paiement mobile instantané</div>
          </div>
          <div style="font-size:11px;color:#1E90FF">→</div>
        </button>

        <!-- Orange Money -->
        <button onclick="selectPayment('orange','${code}')" style="display:flex;align-items:center;gap:14px;background:#1A0F00;border:1px solid #FF6B0030;border-radius:12px;padding:14px 16px;cursor:pointer;font-family:inherit;width:100%;text-align:left">
          <div style="width:40px;height:40px;border-radius:10px;background:#FF6B0020;display:flex;align-items:center;justify-content:center;font-size:20px;flex-shrink:0">🟠</div>
          <div style="flex:1">
            <div style="font-size:13px;font-weight:700;color:#F0F2F8">Orange Money</div>
            <div style="font-size:10px;color:#555;margin-top:2px">Transfert Orange Money</div>
          </div>
          <div style="font-size:11px;color:#FF6B00">→</div>
        </button>

        <!-- Crypto -->
        <button onclick="selectPayment('crypto','${code}')" style="display:flex;align-items:center;gap:14px;background:#0A1A0A;border:1px solid #F7931A30;border-radius:12px;padding:14px 16px;cursor:pointer;font-family:inherit;width:100%;text-align:left">
          <div style="width:40px;height:40px;border-radius:10px;background:#F7931A20;display:flex;align-items:center;justify-content:center;font-size:20px;flex-shrink:0">₿</div>
          <div style="flex:1">
            <div style="font-size:13px;font-weight:700;color:#F0F2F8">Crypto</div>
            <div style="font-size:10px;color:#555;margin-top:2px">USDT · BTC · ETH</div>
          </div>
          <div style="font-size:11px;color:#F7931A">→</div>
        </button>

      </div>

      <div style="text-align:center;font-size:10px;color:#2A2A2A">
        Après paiement, ton accès sera renouvelé dans les 5 minutes
      </div>
...
async function showCryptoPayment(code) {
  const modal = document.getElementById('renewal-modal');
  if (!modal) return;
  const inner = modal.querySelector('[style*="border-radius:20px"]');
  if (!inner) return;

  inner.innerHTML = `
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:18px">
      <button onclick="showRenewalModal()" style="background:#1A1A1A;border:none;border-radius:50%;width:28px;height:28px;color:#555;font-size:14px;cursor:pointer">←</button>
      <div><div style="font-size:14px;font-weight:800;color:#F0F2F8">₿ Crypto</div><div style="font-size:10px;color:#555;margin-top:1px">Paiement automatique — 100 USDT</div></div>
      <button onclick="document.getElementById('renewal-modal').remove()" style="background:#1A1A1A;border:none;border-radius:50%;width:28px;height:28px;color:#555;font-size:14px;cursor:pointer;margin-left:auto">✕</button>
    </div>
    <div style="text-align:center;padding:30px 0;color:#555;font-size:13px">
      <div style="font-size:24px;margin-bottom:10px">⏳</div>
      Génération de l'adresse...
    </div>`;

  try {
    const r = await fetch(SERVER + '/api/payment/create', {
      method: 'POST',
      headers: {'Content-Type':'application/json'},
      body: JSON.stringify({ currency: 'USDTTRC20', code })
    });
    const d = await r.json();
    if (!d.ok || !d.address) throw new Error(d.error||'Failed');

    // Store paymentId to poll
    const paymentId = d.paymentId;
    inner.innerHTML = `
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:18px">
        <button onclick="showRenewalModal()" style="background:#1A1A1A;border:none;border-radius:50%;width:28px;height:28px;color:#555;font-size:14px;cursor:pointer">←</button>
        <div><div style="font-size:14px;font-weight:800;color:#F0F2F8">₿ Crypto</div><div style="font-size:10px;color:#555">USDT TRC-20 · 100 USDT</div></div>
        <button onclick="document.getElementById('renewal-modal').remove()" style="background:#1A1A1A;border:none;border-radius:50%;width:28px;height:28px;color:#555;font-size:14px;cursor:pointer;margin-left:auto">✕</button>
      </div>

      <div style="background:#111;border:1px solid #1A1A1A;border-radius:12px;padding:14px;margin-bottom:12px">
        <div style="font-size:9px;color:#555;letter-spacing:1px;margin-bottom:6px">ADRESSE USDT TRC-20</div>
        <div style="font-family:monospace;font-size:11px;color:#F7931A;word-break:break-all;line-height:1.6">${d.address}</div>
        <div style="margin-top:8px;display:flex;justify-content:space-between;align-items:center">
          <span style="font-size:9px;color:#555">Montant exact:</span>
          <span style="font-family:monospace;font-size:14px;font-weight:700;color:#F0F2F8">${d.amount} ${d.currency}</span>
        </div>
      </div>

      <div style="background:#0A0A0A;border-radius:10px;padding:12px;margin-bottom:14px;text-align:center">
        <div style="font-size:9px;color:#444;margin-bottom:4px">TON CODE</div>
        <div style="font-family:monospace;font-size:18px;font-weight:700;color:#C9A84C;letter-spacing:2px">${code}</div>
      </div>

      <div id="crypto-status" style="text-align:center;font-size:11px;color:#555;margin-bottom:12px;padding:10px;background:#0A0A0A;border-radius:8px">
        ⏳ En attente de paiement...
      </div>

      <div style="text-align:center;font-size:9px;color:#2A2A2A;padding-bottom:10px">
        Envoie exactement le montant indiqué — confirmation automatique en 5 min
      </div>`;
    ...
    const poll = setInterval(async () => {
      polls++;
      if (polls > 60) { clearInterval(poll); return; } // stop after 30 min
      try {
        const pr = await fetch(SERVER + '/api/payment/status/' + paymentId);
        const pd = await pr.json();
        const statusEl = document.getElementById('crypto-status');
        if (pd.status === 'finished' || pd.status === 'confirmed') {
          clearInterval(poll);
          // Update local expiry
          localStorage.setItem('14t_access_exp', String(Date.now() + 30*24*60*60*1000));
          const rm = document.getElementById('renewal-modal');
          if (rm) rm.remove();
          const rb = document.getElementById('renewal-banner-top');
          if (rb) rb.remove();
          const toast = document.createElement('div');
          toast.style.cssText = 'position:fixed;top:20px;left:50%;transform:translateX(-50%);background:#00C87A;color:#000;border-radius:10px;padding:12px 20px;font-size:13px;font-weight:700;z-index:2000';
          toast.textContent = '✅ Paiement confirmé — 30 jours renouvelés !';
          document.body.appendChild(toast);
          setTimeout(() => toast.remove(), 5000);
        } else if (statusEl) {
          const icons = {'waiting':'⏳','confirming':'🔄','partially_paid':'⚠️'};
          statusEl.textContent = (icons[pd.status]||'⏳') + ' ' + (pd.status||'En attente') + '...';
        }
      } catch(e) {}
    }, 30000); // poll every 30s

  } catch(e) {
    const inner2 = modal.querySelector('[style*="border-radius:20px"]');
    if (inner2) inner2.innerHTML += `<div style="color:#FF3B4E;text-align:center;font-size:11px;margin-top:8px">❌ Erreur: ${e.message}</div>`;
  }
}

function selectPayment(method, code) {
  const PAYMENT_INFO = {
    wave: {
      title: '🌊 Wave',
      color: '#1E90FF',
      instructions: [
        'Ouvre Wave sur ton téléphone',
        'Envoie 57 000 CFA au numéro:',
        '+221 78 825 50 99 (Sadikh)',
        'Note: "Renouvellement ' + code + '"'
      ],
      action: 'Ouvrir Wave',
      link: 'https://wa.me/221788255099?text=' + encodeURIComponent('Bonjour Sadikh, je viens de payer 57 000 CFA via Wave pour renouveler mon accès 30 jours. Mon code: ' + code)
    },
    orange: {
      title: '🟠 Orange Money',
      color: '#FF6B00',
      ...
      action: 'Confirmer sur WhatsApp',
      link: 'https://wa.me/221788255099?text=' + encodeURIComponent('Bonjour Sadikh, je viens de payer 57 000 CFA via Orange Money pour renouveler mon accès 30 jours. Mon code: ' + code)
    },
    crypto: {
      title: '₿ Crypto',
      color: '#F7931A',
      instructions: [
        'Envoie en USDT (TRC-20):',
        'Montant: 100 USDT',
        'Adresse: TWallet14TradesXXXXXXXXX',
        'Envoie le TxID sur WhatsApp'
      ],
      action: 'Envoyer TxID',
      link: 'https://wa.me/221788255099?text=' + encodeURIComponent('Bonjour Sadikh, je viens de payer en crypto pour renouveler mon accès 30 jours. Mon code: ' + code + '. TxID: ')
    }
  };

  // Crypto uses NOWPayments automatic flow
  if (method === 'crypto') {
    showCryptoPayment(code);
    return;
  }
  ...
  modal.querySelector('[style*="border-radius:20px"]').innerHTML = `
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:20px">
      <button onclick="showRenewalModal()" style="background:#1A1A1A;border:none;border-radius:50%;width:32px;height:32px;color:#555;font-size:16px;cursor:pointer;flex-shrink:0">←</button>
      <div>
        <div style="font-size:15px;font-weight:800;color:#F0F2F8">${info.title}</div>
        <div style="font-size:10px;color:#555;margin-top:1px">Instructions de paiement</div>
      </div>
      <button onclick="document.getElementById('renewal-modal').remove()" style="background:#1A1A1A;border:none;border-radius:50%;width:32px;height:32px;color:#555;font-size:16px;cursor:pointer;margin-left:auto;flex-shrink:0">✕</button>
    </div>

    <div style="background:#111;border:1px solid #1A1A1A;border-radius:12px;padding:16px;margin-bottom:16px">
      ${info.instructions.map((line, i) => `
        <div style="display:flex;align-items:center;gap:10px;${i>0?'margin-top:10px':''}">
          <div style="width:20px;height:20px;border-radius:50%;background:${info.color}20;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:700;color:${info.color};flex-shrink:0">${i+1}</div>
          <div style="font-size:${i===2?'14px':'11px'};font-weight:${i===2?'700':'400'};color:${i===2?'#F0F2F8':'#888'}">${line}</div>
        </div>`).join('')}
    </div>

    <div style="background:#0A0A0A;border-radius:10px;padding:12px;margin-bottom:16px;text-align:center">
      <div style="font-size:9px;color:#444;margin-bottom:4px">TON CODE À MENTIONNER</div>
      <div style="font-family:monospace;font-size:18px;font-weight:700;color:#C9A84C;letter-spacing:2px">${code}</div>
    </div>

    <a href="${info.link}" target="_blank" rel="noopener"
       style="display:block;width:100%;background:${info.color};color:#000;border:none;border-radius:12px;padding:14px;font-size:13px;font-weight:800;cursor:pointer;text-align:center;text-decoration:none;margin-bottom:10px">
      ✅ ${info.action} — confirmer paiement
    </a>

    <div style="text-align:center;font-size:10px;color:#2A2A2A;padding-bottom:20px">
      Sadikh confirme et active ton accès dans les 5 minutes
    </div>`;
}
...
function showBroadcastMessage(msg, type, link) {
  if (!msg) return;
  const ts = Date.now();
  // Save to localStorage — survives reloads and gives checkSavedBroadcast a fallback
  localStorage.setItem('14t_broadcast', JSON.stringify({ msg, ts: String(ts), dismissed: false, type: type||null, link: link||null }));
  // Always render, with a short delay so the DOM/main-app visibility has
  // settled — don't gate on a one-shot visibility check that can race
  // against the boot sequence and silently show nothing.
  setTimeout(() => renderBroadcastBanner(msg, ts, type, link), 400);
}

function renderBroadcastBanner(msg, ts, type, link) {
  if (!msg) return;
  ...
  const labelEl  = document.getElementById('bc-card-label');
  const linkEl   = document.getElementById('bc-card-link');
  const linkTxt  = document.getElementById('bc-card-link-text');
  if (!card || !msgEl) return;
  msgEl.textContent  = msg;
  card.style.display = 'block';
  if (timeEl) timeEl.textContent = ts ? getTimeAgo(ts) : '';
  if (labelEl) {
    labelEl.textContent = type === 'reflection' ? '📓 Réflexion · Sadikh' : '📢 Message · Sadikh';
  }
  if (linkEl) {
    if (link) {
      linkEl.href = link;
      linkEl.style.display = 'flex';
      if (linkTxt) linkTxt.textContent = type === 'reflection' ? 'Ouvrir mon journal →' : 'Ouvrir →';
    } else {
...
function dismissBroadcast() {
  ...
  // Mark as dismissed — new broadcasts will still show (different ts)
...
function checkURLBroadcast() {
  try {
    ...
    if (bc) {
      ...
      const bcT  = params.get('bct') || '';
      const bcL  = params.get('bcl') || '';
      showBroadcastMessage(msg, bcT, bcL);
      // Clean URL
      window.history.replaceState({}, '', '/');
    }
  } catch(e) {}
...
function checkSavedBroadcast() {
  try {
    ...
    if (age < 48 * 60 * 60 * 1000) {
      setTimeout(() => renderBroadcastBanner(saved.msg, saved.ts, saved.type, saved.link), 800);
    } else {
...
async function fetchLatestBroadcast() {
  try {
    ...
    const d = await r.json();
    // Server sends d.broadcast (single) — normalize to array
    const bc = d.broadcast || null;
    if (!bc) { const card=document.getElementById('bc-card'); if(card) card.style.display='none'; return; }
    const latest = { id: String(bc.ts||bc.id||0), message: bc.message, title: bc.title||'', ts: bc.ts };
    // Check if already dismissed
    const dismissed = localStorage.getItem('14t_bc_dismissed');
    if (dismissed === latest.id) return;
    renderBroadcastBanner(latest.message || '', latest.ts || new Date(latest.created_at||Date.now()).getTime());
...
function renderAllTrades(trades) {
  ...
  if (ids === renderedTradeIds.join(',')) {
    // Same trade(s) still active — use the lightweight update path.
    // Previously this also required "no hit yet", which meant the FULL
    // card got destroyed and rebuilt on every single poll for the rest
    // of a trade's life once any TP/SL hit — constant DOM thrashing that
    // could make the share button flicker or appear to never show.
    // updateTradeCard() already correctly handles TP status, the BE
    // banner, and share button visibility, so there's no need to force
    // a full re-render just because a hit occurred.
  ...
  `).join('');
  trades.forEach(triggerInitialCalc);
}

// Runs the calculator once on render if a saved capital value exists,
// so returning users see real numbers immediately, not just after
// they type something.
function triggerInitialCalc(trade) {
  if (!trade.entry || !trade.sl || !trade.tp1) return;
  if (localStorage.getItem('14t_calc_capital')) {
    updateSignalCalc(trade.id, trade.entry, trade.sl, trade.tp1);
  }
}

function getFomoLockedCardHTML(trade) {
  const asset   = (trade.asset||'GOLD').toUpperCase();
  const pairs   = {GOLD:'XAU/USD',SILVER:'XAG/USD',BTC:'BTC/USD',NAS100:'NAS100',US30:'DOW JONES'};
  const pips    = trade.pipsInFavor;
  const members = trade.memberCount;
  ...
  if (created) {
    ...
    elapsedTxt = mins < 1 ? "moins d'1 min" : (mins + ' min');
  }

  return `<div class="pw-border" id="card-${trade.id}"><div class="pw-inner" style="padding:26px 20px;display:flex;flex-direction:column;align-items:center;gap:14px;font-family:inherit;background:radial-gradient(ellipse at center,#0C0B00,#060504)">
    <div style="font-size:10px;font-weight:700;color:#3A3A3A;letter-spacing:2px">${asset} · SIGNAL EN COURS</div>

    <div style="display:flex;align-items:center;gap:7px;background:#00C87A0F;border:1px solid #00C87A28;border-radius:20px;padding:5px 14px">
      <span class="pw-livedot" style="background:#00C87A"><i></i></span>
      <div style="font-size:9px;font-weight:700;color:#00C87A;letter-spacing:1px">LIVE · MEMBRES EN PROFIT</div>
    </div>

    ${pips != null ? `
    <div style="text-align:center">
      <div style="font-family:'Space Mono',monospace;font-size:36px;font-weight:800;color:#00C87A">+${pips} pips</div>
      <div style="font-size:9px;color:#444;margin-top:2px">et ça continue de monter…</div>
    </div>` : `
    <div style="text-align:center">
      <div style="font-size:13px;color:#F0F2F8;font-weight:600">Signal actif</div>
      <div style="font-size:9px;color:#444;margin-top:2px">Les membres suivent l'évolution en direct</div>
    </div>`}

    <div style="width:90px;height:90px;border-radius:50%;border:3px solid #444;display:flex;align-items:center;justify-content:center;background:#0A0A0A">
      <span style="font-size:28px">🔒</span>
    </div>

    <div style="text-align:center;padding:0 10px">
      ${elapsedTxt ? `<div style="font-size:12px;color:#F0F2F8;font-weight:600;line-height:1.4">Ce signal est actif depuis <span style="color:#F5A800">${elapsedTxt}</span></div>` : ''}
      <div style="font-size:10px;color:#555;margin-top:4px">Direction et niveaux réservés aux membres</div>
    </div>

    ${members != null ? `<div style="display:flex;align-items:center;gap:6px;font-size:9px;color:#3A3A3A">
      <span>👥</span> ${members} membres suivent ce signal en direct
    </div>` : ''}

    <button onclick="showLogin('trial')" style="width:100%;background:linear-gradient(135deg,#C9A84C,#FFD700);color:#000;border:none;border-radius:11px;padding:13px;font-size:12px;font-weight:800;font-family:inherit;margin-top:4px;cursor:pointer">🔓 Débloquer ce signal</button>

    <div style="font-size:8px;color:#333;text-align:center">7 jours gratuits · Pas de CB</div>
  </div></div>`;
}

function getSigCardHTML(trade) {
  if (!trade.direction) return getFomoLockedCardHTML(trade);
  ...
  const slHit = !!trade.sl_hit;
  const borderMod = slHit ? 'pw-red' : (isBuy ? 'pw-green' : 'pw-red');

  return `<div class="pw-border ${borderMod}" id="card-${trade.id}"><div class="pw-inner" style="display:flex;flex-direction:column;gap:12px;overflow:hidden;width:100%">
    <div class="lt-top-row">
      ...
      <div class="lt-live-badge">
        <span class="pw-livedot" style="background:${color}"><i></i></span>
    ...
    ${trade.entry ? `<div style="display:flex;align-items:center;justify-content:space-between;padding:9px 12px;background:#0A0A0A;border-radius:9px;border:1px solid #1A1A1A">
    ...
    <div style="display:flex;flex-direction:column;gap:5px">${tpRows}</div>
    <div class="lt-sl-row">
    ...
    ${(trade.entry && trade.sl && trade.tp1) ? `
    <!-- Embedded position calculator — real entry/SL/TP1 distances -->
    <div style="position:relative;margin-top:2px">
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px">
        <div style="width:20px;height:1px;background:linear-gradient(90deg,transparent,#C9A84C)"></div>
        <div style="font-size:9px;font-weight:800;color:#C9A84C;letter-spacing:2px">CE TRADE POUR TOI</div>
        <div style="width:20px;height:1px;background:linear-gradient(90deg,#C9A84C,transparent)"></div>
      </div>
      <div style="background:#0A0A0A;border:1px solid #1F1F1F;border-radius:12px;padding:14px">
        <div style="margin-bottom:12px">
          <div style="font-size:9px;color:#666;margin-bottom:6px">Ton capital</div>
          <div style="background:linear-gradient(155deg,#131313,#0D0D0D);border:1px solid #333;border-radius:10px;padding:11px 13px;display:flex;align-items:center;justify-content:space-between">
            <input type="number" id="calc-cap-${trade.id}" inputmode="decimal" placeholder="1000" value="${localStorage.getItem('14t_calc_capital')||''}" oninput="updateSignalCalc('${trade.id}', ${trade.entry}, ${trade.sl}, ${trade.tp1})" style="background:none;border:none;outline:none;font-family:'Space Mono',monospace;font-size:16px;color:#F0F2F8;font-weight:800;width:100%">
            <span style="font-size:10px;color:#555;font-weight:600;flex-shrink:0">USD</span>
          </div>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:12px">
          <div style="background:#131313;border-radius:9px;padding:10px;text-align:center">
            <div style="font-size:7px;color:#555;margin-bottom:3px">LOT SUGGÉRÉ</div>
            <div id="calc-lot-${trade.id}" style="font-family:'Space Mono',monospace;font-size:15px;font-weight:800;color:#F0F2F8">—</div>
          </div>
          <div style="background:#131313;border-radius:9px;padding:10px;text-align:center">
            <div style="font-size:7px;color:#555;margin-bottom:3px">RISQUE</div>
            <div id="calc-riskpct-${trade.id}" style="font-family:'Space Mono',monospace;font-size:15px;font-weight:800;color:#F0F2F8">—</div>
          </div>
        </div>
        <div style="display:flex;gap:8px;margin-bottom:8px">
          <div style="flex:1;background:linear-gradient(155deg,#1A0D0E,#0A0707);border:1px solid #FF3B4E25;border-radius:9px;padding:10px;text-align:center">
            <div style="font-size:7px;color:#FF6B6B;margin-bottom:3px">SI SL TOUCHÉ</div>
            <div id="calc-risk-${trade.id}" style="font-family:'Space Mono',monospace;font-size:16px;font-weight:800;color:#FF6B6B">—</div>
          </div>
          <div style="flex:1;background:linear-gradient(155deg,#0F1F15,#080D0A);border:1px solid #00C87A30;border-radius:9px;padding:10px;text-align:center">
            <div style="font-size:7px;color:#00E68A;margin-bottom:3px">SI TP1 ATTEINT</div>
            <div id="calc-gain-${trade.id}" style="font-family:'Space Mono',monospace;font-size:16px;font-weight:800;color:#00E68A">—</div>
          </div>
        </div>
        <div id="calc-note-${trade.id}" style="text-align:center;font-size:8px;color:#555;line-height:1.5"></div>
      </div>
      <button onclick="openMT5(${JSON.stringify(asset)})" style="width:100%;margin-top:10px;background:linear-gradient(135deg,#00C87A,#00E68A);color:#001A0F;border:none;border-radius:12px;padding:13px;font-size:13px;font-weight:800;font-family:inherit;box-shadow:0 4px 20px #00C87A30;cursor:pointer">✓ J'exécute ce trade</button>
    </div>` : ''}

    <div id="share-btn-${trade.id}" style="display:${trade.tp1_hit||trade.tp2_hit||trade.tp3_hit?'block':'none'};margin-top:4px">
      <button onclick="showShareCard('${trade.id}')" style="width:100%;background:${color}10;border:1px solid ${color}25;border-radius:9px;padding:10px;display:flex;align-items:center;justify-content:center;gap:8px;cursor:pointer;font-family:Inter,sans-serif">
...
// Recomputes position sizing live as the user types their capital.
// $1 per point at 0.01 lot (standard gold convention) — real entry/SL/TP1
// distances from the actual signal, not hardcoded numbers.
// Opens MT5 directly to the correct symbol — confirmed working for Gold
// (metatrader5://chart?symbol=XAUUSD). Other symbols use the standard MT5
// naming convention but haven't been individually confirmed the same way.
function openMT5(asset) {
  const MT5_SYMBOLS = {
    GOLD:   'XAUUSD',
    SILVER: 'XAGUSD',
    BTC:    'BTCUSD',
  ...
  const symbol = MT5_SYMBOLS[(asset||'GOLD').toUpperCase()] || 'XAUUSD';
  const url = `metatrader5://chart?symbol=${symbol}`;
  // Try the deep link; if MT5 isn't installed or the scheme doesn't
  // resolve (e.g. testing on desktop), nothing visibly breaks — just
  // silently does nothing, which is an acceptable fallback here.
  window.location.href = url;
}

function updateSignalCalc(tradeId, entry, sl, tp1) {
  const capEl = document.getElementById('calc-cap-' + tradeId);
  const capital = parseFloat(capEl?.value);
  if (capital > 0) localStorage.setItem('14t_calc_capital', capital);
  const lotEl = document.getElementById('calc-lot-' + tradeId);
  const riskPctEl = document.getElementById('calc-riskpct-' + tradeId);
  const riskEl = document.getElementById('calc-risk-' + tradeId);
  const gainEl = document.getElementById('calc-gain-' + tradeId);
  const noteEl = document.getElementById('calc-note-' + tradeId);
  if (!capital || capital <= 0) {
    [lotEl,riskPctEl,riskEl,gainEl].forEach(el => { if (el) el.textContent = '—'; });
  ...
  const DOLLAR_PER_POINT_PER_001LOT = 1; // $1 per point at 0.01 lot

  const targetRiskUsd = capital * 0.01; // aim for 1% risk
  let lot = (targetRiskUsd / (slPoints * DOLLAR_PER_POINT_PER_001LOT)) * 0.01;
  lot = Math.max(0.01, Math.round(lot * 100) / 100); // round to nearest 0.01, minimum 0.01

  const riskUsd = (lot / 0.01) * slPoints * DOLLAR_PER_POINT_PER_001LOT;
  const gainUsd = (lot / 0.01) * tpPoints * DOLLAR_PER_POINT_PER_001LOT;
  const riskPct = (riskUsd / capital) * 100;

  if (lotEl) lotEl.textContent = lot.toFixed(2);
  if (riskPctEl) { riskPctEl.textContent = riskPct.toFixed(1) + '%'; riskPctEl.style.color = riskPct > 2.5 ? '#FFD700' : '#F0F2F8'; }
  if (riskEl) riskEl.textContent = '-$' + riskUsd.toFixed(2);
  if (gainEl) gainEl.textContent = '+$' + gainUsd.toFixed(2);
  if (noteEl) {
    const ratio = (tpPoints / slPoints).toFixed(2);
    noteEl.textContent = riskPct > 2.5
      ? `Ratio ${ratio}:1 · Note : 0.01 est le lot minimum — à ce capital ça représente ${riskPct.toFixed(1)}%, un peu au-dessus du 1-2% recommandé`
      : `Ratio ${ratio}:1 — taille adaptée à ton capital`;
  }
}
...
// ── Share Card ─────────────────────────────────────────────────────
function showShareCard(tradeId) {
  const trades = lastState?.activeTrades || (lastState?.activeTrade ? [lastState.activeTrade] : []);
  const trade  = (tradeId ? trades.find(t => t.id === tradeId) : null) || trades[0];
  if (!trade) return;
  // Real entry/TP/SL are already numbers on a live trade object — compute
  // the real point distance directly, same reasoning as the historical
  // version: targets vary trade to trade, not a fixed pip value per level.
  // 1 point = 10 pips on gold — multiply by 10, don't divide back down.
  const tp = trade.tp3_hit ? 3 : trade.tp2_hit ? 2 : trade.tp1_hit ? 1 : 0;
  if (trade.entry != null) {
    if (tp > 0 && trade['tp'+tp] != null) {
      trade._realPipsOverride = Math.round(Math.abs(trade['tp'+tp] - trade.entry) * 10);
    } else if (trade.sl_hit && trade.sl != null) {
      trade._realPipsOverride = -Math.round(Math.abs(trade.sl - trade.entry) * 10);
    } else if (trade.be_hit) {
      trade._realPipsOverride = 0;
    }
  }
  populateShareCard(trade);
}

// Historical signals (from signals_log / track-record) have a different,
// more limited shape than live trades — no entry/SL/TP price levels
// stored, just type/level/asset/direction/timestamp. This adapts one
// into the shape populateShareCard() expects, so history items can be
// shared the same way a live TP hit can.
function showShareCardFromHistory(sig) {
  if (!sig) return;
  const level = sig.level || 1;
  // Real point distance when available (entry_price/exit_price were only
  // added recently — older entries won't have these, and fall back to
  // the card's existing "—" placeholder for entry/SL/TP display, while
  // populateShareCard falls back to an approximate table for the pips
  // number specifically since targets vary trade to trade, not fixed).
  let realPips = null;
  if (sig.entry_price != null && sig.exit_price != null) {
    const entryNum = parseFloat(sig.entry_price);
    const exitNum  = parseFloat(sig.exit_price);
    if (!isNaN(entryNum) && !isNaN(exitNum)) {
      // 1 point = 10 pips on gold (e.g. entry 2418.50 → TP 2438.50 is a
      // 20.00 point move = 200 pips) — multiply by 10, don't divide back.
      realPips = Math.round(Math.abs(exitNum - entryNum) * 10);
      if (sig.type === 'sl_hit') realPips = -realPips;
      if (sig.type === 'be_hit') realPips = 0;
    }
  }
  const adapted = {
    id:       'hist-' + sig.timestamp,
    asset:    sig.asset,
    direction: sig.direction,
    tp1_hit:  sig.type === 'tp_hit' && level >= 1,
    tp2_hit:  sig.type === 'tp_hit' && level >= 2,
    tp3_hit:  sig.type === 'tp_hit' && level >= 3,
    sl_hit:   sig.type === 'sl_hit',
    be_hit:   sig.type === 'be_hit',
    entry: sig.entry_price || null,
    sl: sig.type === 'sl_hit' ? sig.exit_price : null,
    tp1: sig.type === 'tp_hit' && level === 1 ? sig.exit_price : null,
    tp2: sig.type === 'tp_hit' && level === 2 ? sig.exit_price : null,
    tp3: sig.type === 'tp_hit' && level === 3 ? sig.exit_price : null,
    _historicalDate: sig.timestamp,
    _realPipsOverride: realPips,
  };
  populateShareCard(adapted);
}

function populateShareCard(trade) {
  const isBuy  = (trade.direction||'').toUpperCase() === 'BUY';
  const asset  = (trade.asset||'GOLD').toUpperCase();
  const pairs  = {GOLD:'XAU/USD', SILVER:'XAG/USD', BTC:'BTC/USD', NAS100:'NAS100', US30:'US30'};
  const green  = '#00E68A';
  const red    = '#FF5C6E';

  // What TP was hit?
  const tp = trade.tp3_hit ? 3 : trade.tp2_hit ? 2 : trade.tp1_hit ? 1 : 0;
  const isSL   = trade.sl_hit;
  const isBE   = trade.be_hit;
  const hitVal = tp > 0 ? trade['tp'+tp] : trade.sl;
  // Real point distance when available (targets vary trade to trade —
  // sometimes 50 points, sometimes 100 — so a fixed pip value per TP
  // level was never actually accurate). Falls back to an approximate
  // table only for older entries logged before real prices were captured.
  const TP_PIPS = {1:120, 2:200, 3:350};
  let pips;
  if (trade._realPipsOverride != null) {
    pips = trade._realPipsOverride;
  } else {
    pips = isSL ? -120 : isBE ? 0 : TP_PIPS[tp] || 0;
  }
  const pipsStr = pips >= 0 ? '+' + pips : String(pips);
  const resultColor = pips >= 0 ? (pips===0?'#888':green) : red;

  // Badge
  let badge = tp===3?'TP3 HIT ✅':tp===2?'TP2 HIT ✅':tp===1?'TP1 HIT ✅':isSL?'SL HIT ❌':isBE?'BE ⚖️':'RUNNING';

  // Date — use the real historical date if this is a past trade, else today
  const d = trade._historicalDate ? new Date(trade._historicalDate) : new Date();
  const dateStr = d.getDate() + ' ' + d.toLocaleString('fr-FR',{month:'short'}) + ' ' + d.getFullYear();

  // Session
  const sess = trade.window || '';

  const set = (id, val) => { const el=document.getElementById(id); if(el) el.textContent=val; };
  const setStyle = (id, prop, val) => { const el=document.getElementById(id); if(el) el.style[prop]=val; };
  const setColor = (id, color) => setStyle(id, 'color', color);

  // Fill card
  set('sc-result-badge', badge);
  setStyle('sc-result-badge', 'background', pips>=0?green+'18':red+'18');
  setStyle('sc-result-badge', 'borderColor', pips>=0?green+'35':red+'35');
  setColor('sc-result-badge', pips>=0?green:red);
  set('sc-date', dateStr);

  set('sc-pill', pairs[asset] || asset);
  set('sc-asset', asset);
  set('sc-dir-lbl', trade.direction || '');
  setColor('sc-dir-lbl', isBuy?green:red);
  set('sc-session', sess);

  // Big pips number
  set('sc-pips', pipsStr);
  setColor('sc-pips', pips===0?'#999':resultColor);

  // Entry / SL / TP
  set('sc-entry', trade.entry || '—');
  set('sc-sl', trade.sl || '—');
  setColor('sc-sl', isSL?red:'#BBB');
  set('sc-sl-lbl', isSL?'SL ✗':'SL');
  setColor('sc-sl-lbl', isSL?red:'#BBB');

  const tpLblEl = document.getElementById('sc-tp-lbl');
  const tpValEl = document.getElementById('sc-tp-val');
  if (tp > 0) {
    if (tpLblEl) { tpLblEl.textContent = 'TP'+tp+' ✓'; tpLblEl.style.color = green; }
    if (tpValEl) { tpValEl.textContent = hitVal||'—'; tpValEl.style.color = green; }
  } else {
    if (tpLblEl) { tpLblEl.textContent = 'TP1'; tpLblEl.style.color = '#BBB'; }
    if (tpValEl) { tpValEl.textContent = trade.tp1||'—'; tpValEl.style.color = '#BBB'; }
  }

  // Stats — real values
  const wr  = localStorage.getItem('14t_wr')      || document.getElementById('stat-winrate')?.textContent  || '89%';
  const sig = localStorage.getItem('14t_signals')  || document.getElementById('stat-signals')?.textContent || '73';
  const str = localStorage.getItem('14t_streak')   || streak || '';
  set('sc-wr',     wr.includes('%') ? wr : wr+'%');
  set('sc-sigs',   sig);
  set('sc-streak', str ? '🔥 '+str+'j' : '—');
...
async function doNativeShare() {
  const cardEl = document.getElementById('share-card-el');
  const pips   = document.getElementById('sc-pips')?.textContent || '';
  const asset  = document.getElementById('sc-asset')?.textContent || 'GOLD';
  const badge  = document.getElementById('sc-result-badge')?.textContent || '';
  const text   = `14⚡ TRADES ALERT\n${asset} · ${badge}\n${pips} pips\n\nSignaux Gold en temps réel. Demande ton accès:\nhttps://14trades-pwa.vercel.app`;

  // Try to capture the real visual card as an image
  if (cardEl && typeof html2canvas === 'function') {
    try {
      ...
      if (blob) {
        const file = new File([blob], `14trades-${asset.toLowerCase()}.png`, { type: 'image/png' });
        // Native share sheet WITH the actual image attached — works for TikTok/WhatsApp/Instagram
        if (navigator.canShare && navigator.canShare({ files: [file] })) {
          await navigator.share({ title: '14 Trades Alert', text, files: [file] });
          return;
        }
        // No native file-share support — download the image directly instead
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url; a.download = file.name;
        document.body.appendChild(a); a.click(); a.remove();
        URL.revokeObjectURL(url);
        showToast('📸 Image téléchargée — partage-la où tu veux');
        return;
      }
    } catch(e) { /* fall through to text-only sharing below */ }
  }

  // Fallback — text only (no image capture available)
  if (navigator.share) {
    navigator.share({ title:'14 Trades Alert', text }).catch(()=>{});
  } else {
...
async function savePlayerIdToServer(code) {
  try {
    ...
    console.log('[PUSH DEBUG] savePlayerIdToServer called. OneSignal exists:', !!window.OneSignal, '| playerId found:', playerId || 'NONE');
    if (!playerId) return;
    localStorage.setItem('14t_player_id', playerId);
    // Send to server
    const resp = await fetch(SERVER + '/api/save-player-id', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ code: code || localStorage.getItem('14t_access'), playerId })
    });
    const respData = await resp.json().catch(() => ({}));
    console.log('[PUSH] Player ID saved:', playerId, '| Server response:', resp.status, respData);
  } catch(e) { console.warn('[PUSH DEBUG] savePlayerIdToServer error:', e); }
}
...
async function fetchStatsFromServer() {
  try {
    const r = await fetch(SERVER + '/api/stats?t=' + Date.now(), { cache: 'no-store' });
    const d = await r.json();
    if (!d.ok) return;
    ...
    // Use server value if valid, else fall back to cached localStorage value
    const cachedWr   = parseInt(localStorage.getItem('14t_wr'))   || 0;
    const cachedSigs = parseInt(localStorage.getItem('14t_signals')) || 0;
    const displayWr   = wr   > 0 ? wr   : cachedWr;
    const displaySigs = sigs > 0 ? sigs : cachedSigs;
    if (wrEl)   wrEl.textContent   = displayWr   > 0 ? displayWr + '%' : '—';
    if (sigsEl) sigsEl.textContent = displaySigs > 0 ? displaySigs     : '—';
    // Cache for next time
    if (wr   > 0) localStorage.setItem('14t_wr',      wr);
    if (sigs > 0) localStorage.setItem('14t_signals',  sigs);

    // Save globals
    if (sigs > 0) { totalSigs = sigs; localStorage.setItem('14t_signals', sigs); }
    if (w > 0)    { wins = w;         localStorage.setItem('14t_wins', w); }

    _statsFetched = true;
  } catch(e) {
    console.error('[fetchStatsFromServer]', e);
    // Retry once after 3s if failed
    if (!_statsFetched) setTimeout(fetchStatsFromServer, 3000);
  }
}
...
// ── NOTIFICATION PERMISSION — soft banner first, not a cold native
// popup. Showing context before the browser's own permission dialog is
// well-documented to significantly improve opt-in rates. This merges the
// banner UX from an earlier version with the permanent (not session-only)
// localStorage tracking from a second version that had accidentally
// overridden it — same function name, only one should ever have existed.
function showNotifPrompt() {
  const already = localStorage.getItem('push_subscribed');
  const denied  = localStorage.getItem('push_denied');
  ...
  if (Notification.permission === 'denied') {
    localStorage.setItem('push_denied', '1');
    return;
  }
...
async function requestPush() {
  showToast('DEBUG: requestPush() appelé', 'info');
  try {
    // Make sure OneSignal is initialized
    if (typeof initOneSignal !== 'undefined') {
      showToast('DEBUG: appel initOneSignal...', 'info');
      await initOneSignal();
    }

    // Short delay to let OneSignal fully init
    await new Promise(r => setTimeout(r, 800));

    showToast('DEBUG: OneSignal existe=' + !!window.OneSignal + ' Notifications=' + !!(window.OneSignal && window.OneSignal.Notifications), 'info');

    if (window.OneSignal && typeof window.OneSignal.Notifications !== 'undefined') {
      // SDK v16+
      showToast('DEBUG: appel requestPermission SDK v16...', 'info');
      await window.OneSignal.Notifications.requestPermission();
      showToast('DEBUG: requestPermission terminé', 'info');
      handlePushResult();

    } else if (window.OneSignalDeferred) {
      // Deferred queue
      showToast('DEBUG: via OneSignalDeferred queue', 'info');
      ...
      showToast('DEBUG: fallback natif', 'info');
      await Notification.requestPermission();
      handlePushResult();
    }
  } catch(e) {
    showToast('DEBUG ERROR: ' + e.message, 'warn');
    // Last resort native
    try {
      await Notification.requestPermission();
      handlePushResult();
    } catch(e2) {
      showToast('Active les notifs dans Réglages téléphone', 'info');
    }
  }
}

// Checks the REAL resulting permission state and gives clear feedback
// either way — success, or exactly what to do if it's still blocked.
// Every code path in requestPush() funnels through here, so nobody ever
// taps the bell and sees nothing happen with no explanation.
function handlePushResult() {
  ...
  if (perm === 'granted') {
    ...
    showToast('Notifications activées ✓', 'buy');
  } else if (perm === 'denied') {
    localStorage.setItem('push_denied', '1');
    showToast('Bloqué — active dans les réglages de ton téléphone', 'info');
  }
  // if 'default' (user just dismissed without deciding), stay silent —
  // they can tap the bell again anytime
}
...
async function fetchState() {
  try {
    ...
    const _code = localStorage.getItem('14t_access') || '';
    const res = await fetch(SERVER + '/api/state?t=' + Date.now() + '&code=' + encodeURIComponent(_code), {
...
document.querySelectorAll('[data-secondary]').forEach(function(el) {
  if (el.dataset.pwDisplay === undefined) el.dataset.pwDisplay = el.style.display || '';
});
['mood-session-strip','referral-banner'].forEach(function(id) {
  var el = document.getElementById(id);
  if (el && el.dataset.pwDisplay === undefined) el.dataset.pwDisplay = el.style.display || '';
});
function setSecondary(show) {
  const ids = ['mood-session-strip','referral-banner'];
  ids.forEach(function(id) {
    var el = document.getElementById(id);
    if (el) el.style.display = show ? el.dataset.pwDisplay : 'none';
  });
  document.querySelectorAll('[data-secondary]').forEach(function(el) {
    if (el.dataset.pwDisplay === undefined) el.dataset.pwDisplay = el.style.display || '';
    if (show) {
      el.style.display = el.dataset.pwDisplay;
    } else {
...
function applyState(d) {
  // Re-run stat displays that depend on lastState.signals — d IS the fresh
  // state, and lastState is set to it by fetchState() right before this runs,
  // so these now have real data to work with on every poll, not just boot.
  try { updateStats(); } catch(e) {}
  try { loadWeeklyRecap(); } catch(e) {}
  ...
  if (broadcast && broadcast.message) {
    const bcTs    = String(broadcast.ts || '');
    const saved   = JSON.parse(localStorage.getItem('14t_broadcast') || '{}');
    const savedTs = String(saved.ts || '');
    const isNew   = bcTs && bcTs !== savedTs;   // new broadcast has different timestamp
    if (bcCard) {
      renderBroadcastBanner(broadcast.message, broadcast.ts, broadcast.type, broadcast.link);
      if (isNew) {
        // New broadcast — always show, reset dismissed state
        localStorage.setItem('14t_broadcast', JSON.stringify({ts:bcTs, msg:broadcast.message, dismissed:false, type:broadcast.type||null, link:broadcast.link||null}));
        bcCard.style.display = 'block';
      } else if (!saved.dismissed) {
        // Same broadcast, not dismissed
        bcCard.style.display = 'block';
      } else {
        // Same broadcast, user dismissed it
        bcCard.style.display = 'none';
      }
    }
  } else if (bcCard) {
    // No broadcast from server — hide card
    bcCard.style.display = 'none';
  }
  ...
  if (!_statsFetched) fetchStatsFromServer();
...
function updateStats() {
  // Prefer the live signals array from the already-proven /api/state
  // polling pipeline over the cached local totals, when available.
  let displayWr = totalSigs > 0 ? Math.round((wins / totalSigs) * 100) : 0;
  let displaySigs = totalSigs;
  let dbgNote = 'lastState=' + (lastState ? 'set' : 'NULL');
  if (lastState && Array.isArray(lastState.signals)) {
    dbgNote += ', raw=' + lastState.signals.length;
    const outcomes = lastState.signals.filter(e => e.type === 'tp_hit' || e.type === 'sl_hit' || e.type === 'be_hit');
    dbgNote += ', outcomes=' + outcomes.length;
    if (outcomes.length > 0) {
    const liveWins = outcomes.filter(e => e.type === 'tp_hit').length;
      displaySigs = outcomes.length;
      displayWr   = Math.round((liveWins / outcomes.length) * 100);
    }
  } else if (lastState) {
    dbgNote += ', signals=' + typeof lastState.signals;
  }
  const wr = document.getElementById('stat-winrate'); if(wr) wr.textContent = displayWr + '%';
  const sg = document.getElementById('stat-signals'); if(sg) sg.textContent = displaySigs;
  const dbgEl1 = document.getElementById('temp-debug-line');
  if (dbgEl1) dbgEl1.textContent = 'updateStats: ' + dbgNote;
  animatePnL(totalPnL, todayPnL);
}
...
function updateMemberSince() {
  ...
  const std = document.getElementById('stat-days'); if(std) std.textContent = days > 0 ? days + 'j' : '1j';
}
...
function startLivePrices() { fetchLivePrices(); setInterval(fetchLivePrices, 30000); }

// ── Weekly recap card — real signals_log data, this calendar week ──
async function loadWeeklyRecap() {
  const card = document.getElementById('weekly-recap-card');
  if (!card) return;
  const dbg = document.getElementById('temp-debug-line');
  try {
    // Use the already-proven /api/state polling pipeline (lastState) instead
    // of a separate fetch — sidesteps whatever is blocking the standalone
    // /api/track-record call for some sessions.
    let entries = (lastState && Array.isArray(lastState.signals)) ? lastState.signals : null;
    if (!entries) {
      // lastState not populated yet — force one fetch now
      if (dbg) dbg.textContent = 'loadWeeklyRecap: lastState vide, fetch forcé…';
      await fetchState();
      entries = (lastState && Array.isArray(lastState.signals)) ? lastState.signals : [];
    }
    entries = entries.filter(e => e.type === 'tp_hit' || e.type === 'sl_hit' || e.type === 'be_hit');
    // Start of this week (Monday 00:00, local device time)
    const now = new Date();
    ...
    const sigCount = thisWeek.length;
    const wins   = thisWeek.filter(e => e.type === 'tp_hit').length;
    const losses = thisWeek.filter(e => e.type === 'sl_hit').length;

    // Progress bar segments, chronological, capped at 8 for layout sanity
    const shown = thisWeek.slice(-8);
    const progRow = document.getElementById('wr-progress-row');
    const progLbl = document.getElementById('wr-progress-labels');
    if (progRow) {
      progRow.innerHTML = shown.length
        ? shown.map(e => {
            const isWin = e.type === 'tp_hit';
            const isBe  = e.type === 'be_hit';
            const color = isWin ? '#00C87A' : isBe ? '#C9A84C' : '#2A1414';
            const glow  = isWin ? 'box-shadow:0 0 12px #00C87A60' : isBe ? 'box-shadow:0 0 8px #C9A84C40' : '';
            return `<div style="flex:1;height:6px;border-radius:3px;background:${color};${glow}"></div>`;
          }).join('')
        : `<div style="flex:1;height:6px;border-radius:3px;background:#1A1A1A"></div>`;
    }
    if (progLbl) {
      progLbl.innerHTML = shown.length
        ? shown.map(e => {
            const isWin = e.type === 'tp_hit';
            const isBe  = e.type === 'be_hit';
            const label = isWin ? 'Win' : isBe ? 'BE' : 'Loss';
            const style = isWin ? '' : isBe ? 'color:#C9A84C' : 'color:#663333';
            return `<span style="${style}">${label}</span>`;
          }).join('')
        : `<span>Pas encore de signal cette semaine</span>`;
    }

    const set = (id, val) => { const el = document.getElementById(id); if (el) el.textContent = val; };
    set('wr-sig-count', sigCount + (sigCount === 1 ? ' SIGNAL' : ' SIGNAUX'));
    set('wr-wins', wins);
    set('wr-losses', losses);

    // Same R:R economics established elsewhere — win ≈ 2.5R, loss = 1R
    const WIN_UNIT = 75, LOSS_UNIT = 30;
    const netUnits = wins * WIN_UNIT - losses * LOSS_UNIT;
    const msgEl = document.getElementById('wr-message');
    const subEl = document.getElementById('wr-submessage');
    if (sigCount === 0) {
      if (msgEl) msgEl.textContent = 'Aucun signal pour l\'instant cette semaine';
      if (subEl) subEl.textContent = 'Reviens dès qu\'un setup se présente';
    } else if (netUnits >= 0) {
      if (msgEl) msgEl.textContent = 'Tu es déjà positif cette semaine';
      if (subEl) subEl.textContent = losses > 0
        ? `${wins} win${wins>1?'s':''} couvre${wins>1?'nt':''} largement ${losses>1?'les pertes':'la perte'}`
        : `${wins} win${wins>1?'s':''} sans perte cette semaine`;
    } else {
      const neededWins = Math.ceil(Math.abs(netUnits) / WIN_UNIT);
      if (msgEl) msgEl.textContent = `Encore ${neededWins} win${neededWins>1?'s':''} pour être positif`;
      if (subEl) subEl.textContent = 'La semaine n\'est pas terminée';
    }

  } catch(e) {
    console.error('[loadWeeklyRecap]', e);
    if (dbg) dbg.textContent = 'loadWeeklyRecap ERROR: ' + (e?.message || String(e));
  }
}

async function fetchLivePrices() {
  // Gold — via our own server's cached endpoint, not goldprice.dev
  // directly. Fixes a real quota problem: every member's app polling
  // goldprice.dev independently every 15s could exhaust its 1,000
  // calls/month free-tier limit within minutes once several people have
  // the app open. The server caches this for everyone, one shared call.
  let goldPrice = null;
  try {
    const r = await fetch(SERVER + '/api/gold-price');
    const d = await r.json();
    goldPrice = d?.ok ? d.price : null;
  } catch(e) { /* fall through to metals.live below */ }

  if (goldPrice != null) {
    const el = document.getElementById('mood-gold-price');
    if (el) el.textContent = '$' + parseFloat(goldPrice).toFixed(2);
    const scoPriceEl = document.getElementById('sco-live-price');
    if (scoPriceEl) scoPriceEl.textContent = parseFloat(goldPrice).toFixed(2);
  } else {
    // Our server's endpoint failed entirely (rare) — use the original source
    try {
      const r = await fetch('https://api.metals.live/v1/spot/gold');
      const d = await r.json();
      const p = d[0]?.price;
  ...
  // Silver — metals.live for now (goldprice.dev's silver support varies by plan)
  try {
    const r = await fetch('https://api.metals.live/v1/spot/silver');
    const d = await r.json();
    const p = d[0]?.price;
...
function checkFOMO(signals){
  ...
  const liveTradeContainer=document.getElementById('live-trade');
  if(liveTradeContainer && liveTradeContainer.innerHTML.trim()!=='')return;
...
function renderHistory(signals){
  ...
  feed.innerHTML=items.map((sig,i)=>{
    ...
    const clickAttr = isClosed ? ` onclick="showShareCardFromHistory(window._historyItems[${i}])" style="cursor:pointer"` : '';
    return`<div class="hist-item"${clickAttr}><div class="hi-left"><div class="hi-ico" style="background:${m.bg};color:${m.color};border:1px solid ${m.border}">${m.ico}</div><div><div class="hi-asset">${sig.asset||'GOLD'}</div><div class="hi-sub">${formatAgo(sig.timestamp)}</div></div></div><div class="hi-right">${sig.direction?`<div class="hi-dir ${(sig.direction||'').toLowerCase()}">${sig.direction}</div>`:''}${out}${isClosed?'<div style="font-size:9px;color:#3A3A3A;margin-top:2px">📤 Partager</div>':''}</div></div>`;
  }).join('');
}
