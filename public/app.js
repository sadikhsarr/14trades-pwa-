...
async function fetchStatsFromServer() {
  try {
    ...
    if (wrEl   && sigs > 0) wrEl.textContent   = wr + '%';
    if (sigsEl && sigs > 0) sigsEl.textContent = sigs;

    // Save to globals
    if (sigs > 0) { totalSigs = sigs; localStorage.setItem('14t_signals', sigs); }
    if (w > 0)    { wins = w;         localStorage.setItem('14t_wins', w); }

    // Days — handled by updateMemberSince
    // P&L — handled by computeStatsFromSignals from /api/state
  } catch(e) {}
}
