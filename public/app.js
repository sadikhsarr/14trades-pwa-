...
function psShowTab(tab) {
  ['signal','home','rank','risk'].forEach(t => {
    ...
    if (page) page.className = 'ps-page' + (t===tab?' a':'');
    if (btn)  btn.className  = 'ps-tab'  + (t===tab?' a':'');
    if (bn) {
      bn.className = 'ps-bn' + (t===tab?' a':'');
      const lbl = bn.querySelector('.ps-bnl');
      if (lbl) lbl.style.color = t===tab ? '#C9A84C' : '';
    }
  });
  if (tab==='rank') psLoadFullLeaderboard();
  if (tab==='risk') { psRgFull(); }
}
...
async function psLoadPublicData() {
  try {
    ...
    const colors = ['#C9A84C','#888','#555','#333','#222'];
    ...
    const html = members.map((m,i) => {
      ...
      const c = colors[i]||'#222';
      const hab = m.habit_days > 0 ? ' · ' + m.habit_days + 'j 🔥' : '';
      const balFmt = bal >= 1000 ? '$' + (bal/1000).toFixed(1) + 'K' : '$' + Math.round(bal);
      return `<div class="ps-mb">
        <div class="ps-mbn" style="color:${c}">${i+1}</div>
        ...
        <div class="ps-mbr"><div class="ps-mbb" style="color:${c}">${balFmt}</div><div class="ps-mbbar"><div class="ps-mbf" style="width:${prog}%;background:${c}"></div></div></div>
      </div>`;
    }).join('');
    const top5 = document.getElementById('ps-top5');
    if (top5) top5.innerHTML = html || '<div style="padding:16px;text-align:center;color:#1A1A1A;font-size:12px">Aucun membre</div>';
    const cnt = document.getElementById('ps-member-count');
    if (cnt) cnt.textContent = (d.members||[]).length + ' membres';

    // Broadcast
    const rb = await fetch(SERVER+'/api/broadcasts');
    const db = await rb.json();
    const bc = (db.broadcasts||[])[0];
    if (bc) {
      const tt = document.getElementById('ps-bc-title');
      const np = document.getElementById('ps-bc-pips');
      const sb = document.getElementById('ps-bc-sub');
      const tg = document.getElementById('ps-bc-tag');
      if(tt) tt.textContent = bc.title || 'Dernier résultat';
      if(np) np.textContent = bc.result || '—';
      if(sb) sb.textContent = bc.message ? bc.message.substring(0,60) : '—';
      if(tg) tg.textContent = bc.emoji ? bc.emoji + ' Résultat confirmé' : '✅ Résultat confirmé';
    }
  } catch(e) {}
}
...
async function psLoadFullLeaderboard() {
  try {
    ...
    const colors = ['#C9A84C','#888','#666','#555','#444','#333','#2A2A2A','#222','#1A1A1A'];
    const PHASES = [{f:200,t:400},{f:400,t:800},{f:800,t:1600},{f:1600,t:3200},{f:3200,t:6400},{f:6400,t:12800},{f:12800,t:25600},{f:25600,t:51200},{f:51200,t:102400},{f:102400,t:204800},{f:204800,t:409600},{f:409600,t:819200},{f:819200,t:1000000}];
    const html = members.map((m,i) => {
      const bal = parseFloat(m.balance);
      const ph  = Math.max(1, m.phase||1);
    ...
    const cnt2 = document.getElementById('ps-member-count2');
    if (cnt2) cnt2.textContent = members.length + ' membres';
  } catch(e) {}
}
