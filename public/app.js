...
async function loadPublicBroadcasts() {
  try {
    ...
    if (stats.ok) {
      ...
      if (sg) sg.textContent = (stats.signals||0) + '+';
      if (mb) mb.textContent = (stats.total||150) + '+';
    }
    const r = await fetch(BROADCAST_SERVER + '/api/broadcasts');
    const d = await r.json();
    const feed = document.getElementById('pub-broadcasts');
    if (!feed) return;
    ...
    const typeColors  = { update:'#555', signal:'#FFD700', result:'#00C87A', competition:'#C9A84C', alert:'#FF3B4E' };
    feed.innerHTML = broadcasts.map(function(b) {
      var icon  = typeIcons[b.type]  || '📢';
      var color = typeColors[b.type] || '#555';
      var date  = new Date(b.created_at).toLocaleDateString('fr-FR', { day:'2-digit', month:'short', hour:'2-digit', minute:'2-digit' });
      return '<div style="background:#0D0D0D;border:1px solid ' + (b.pinned?'#FFD70025':'#1A1A1A') + ';border-radius:12px;padding:16px;margin-bottom:10px">' +
        '<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">' +
          '<span style="font-size:16px">' + icon + '</span>' +
          '<div style="flex:1;font-size:13px;font-weight:600;color:#F0F0F0">' + b.title + '</div>' +
          (b.pinned ? '<span style="font-size:9px;padding:2px 7px;border-radius:10px;background:#FFD70015;color:#FFD700;font-weight:700">PIN</span>' : '') +
        '</div>' +
        '<div style="font-size:13px;color:#888;line-height:1.6;margin-bottom:8px;white-space:pre-wrap">' + b.message + '</div>' +
        '<div style="font-size:10px;color:#333">' + date + '</div>' +
        '</div>';
    }).join('');
  } catch(e) {
    var feed = document.getElementById('pub-broadcasts');
    if (feed) feed.innerHTML = '<div style="text-align:center;padding:32px;color:#333;font-size:13px">Erreur de chargement</div>';
  }
}
