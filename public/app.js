...
async function checkCode() {
  ...
  try {
    ...
    if (d.valid) {
      ...
      error.textContent = '';
      localStorage.setItem('14t_access', val);
      localStorage.setItem('14t_access_exp', new Date(d.expires_at).getTime());
      if (!localStorage.getItem('14t_join')) localStorage.setItem('14t_join', new Date().toISOString());
      showToastAccess(m.success);
      setTimeout(unlockApp, 800);
    } else {
...
function checkAccess() {
  ...
  const expiry = expRaw ? parseInt(expRaw) : 0;
  const now    = Date.now();

  console.log('[checkAccess] code='+saved+' valid='+(expiry>now));

  if (saved && expiry > now) {
    console.log('[checkAccess] VALID → booting app');
    hidePublicScreen();
    bootApp();
    // Failsafe — force main app visible after 2s in case something hides it
    setTimeout(function() {
      var pub  = document.getElementById('public-screen');
      var main = document.getElementById('main-app');
      var saved2 = localStorage.getItem('14t_access');
      var exp2   = parseInt(localStorage.getItem('14t_access_exp')||'0');
      if (saved2 && exp2 > Date.now()) {
  ...
  if (saved) {
    console.log('[checkAccess] has code but expiry issue → revalidating');
    revalidateCode(saved);
    return;
  }

  console.log('[checkAccess] no code → public screen');
  showPublicScreen();
}
...
async function revalidateCode(code) {
  ...
  try {
    ...
    if (d.valid) {
      ...
      console.log('[revalidateCode] VALID → booting app, expiry set to', new Date(exp).toISOString());
      hidePublicScreen();
      bootApp();
    } else {
      console.log('[revalidateCode] INVALID → showing expired screen');
...
async function showPublicScreen() {
  ...
  if (ps) {
    ...
    psLoadPublicData();
    psRunCounters();
    setInterval(psLoadPublicData, 60000);
    setTimeout(() => { if (typeof psRgFull === 'function') psRgFull(); }, 300);
  }
}


function showAccessScreen() {
  window.location.href = 'https://14trades-pwa.vercel.app/access.html?from=public';
}
...
document.addEventListener('DOMContentLoaded', async () => {
  // Small delay to ensure localStorage is fully available after redirect
  setTimeout(function() {
    checkAccess();
  }, 50);

  // Safety fallback — only if no valid code after 1.5 seconds
  setTimeout(function() {
    var saved = localStorage.getItem('14t_access');
    var exp   = parseInt(localStorage.getItem('14t_access_exp')||'0');
    var main  = document.getElementById('main-app');
    var pub   = document.getElementById('public-screen');
    if (!saved || exp < Date.now()) {
      if (pub && pub.style.display === 'none' && main && main.style.display === 'none') {
        showPublicScreen();
      }
    }
  }, 1500);
...
async function bootApp() {
  ...
  checkOnboarding();
  fetchMemberCount();
  // Show notification prompt after 2 seconds if not granted
  setTimeout(() => {
...
function showNotifPrompt() {
  ...
  overlay.style.cssText = 'position:fixed;inset:0;z-index:9999;background:#080808;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:32px 24px;font-family:Inter,system-ui,sans-serif';
  overlay.innerHTML = `
    <div style="max-width:320px;width:100%;text-align:center">
      <div style="font-size:48px;margin-bottom:20px">🔔</div>
      <div style="font-family:monospace;font-size:20px;font-weight:700;color:#C9A84C;margin-bottom:10px">Ne rate plus jamais un signal</div>
      <div style="font-size:13px;color:#555;line-height:1.7;margin-bottom:28px">
        Active les notifications pour recevoir<br>
        les signaux Gold en temps réel.<br><br>
        <span style="color:#333">Entry · TP1 · TP2 · TP3 · SL</span><br>
        <span style="color:#333">Directement sur ton téléphone.</span>
      </div>
      <div style="background:#0D0D0D;border:1px solid #141414;border-radius:12px;padding:14px;margin-bottom:20px">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">
          <div style="width:6px;height:6px;border-radius:50%;background:#00C87A;flex-shrink:0"></div>
          <div style="font-size:12px;color:#F0F0F0;text-align:left">Signal Gold BUY — entre maintenant ⚡</div>
        </div>
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">
          <div style="width:6px;height:6px;border-radius:50%;background:#00C87A;flex-shrink:0"></div>
          <div style="font-size:12px;color:#F0F0F0;text-align:left">✅ TP1 atteint — +150 pips</div>
        </div>
        <div style="display:flex;align-items:center;gap:10px">
          <div style="width:6px;height:6px;border-radius:50%;background:#00C87A;flex-shrink:0"></div>
          <div style="font-size:12px;color:#F0F0F0;text-align:left">🔥 TP3 — Full run +600 pips</div>
        </div>
      </div>
      <button id="notif-yes-btn" style="width:100%;background:linear-gradient(135deg,#C9A84C,#FFD700);color:#000;border:none;border-radius:12px;padding:16px;font-size:15px;font-weight:800;cursor:pointer;font-family:inherit;margin-bottom:10px">
        ⚡ Activer les notifications
      </button>
      <button id="notif-later-btn" style="width:100%;background:none;border:none;color:#333;font-size:12px;cursor:pointer;font-family:inherit;padding:8px">
        Plus tard
      </button>
    </div>
  `;
  ...
  document.getElementById('notif-later-btn').onclick = () => {
    overlay.remove();
    // Show reminder banner after 30 seconds
    setTimeout(showNotifBanner, 30000);
  };
}
