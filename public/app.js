...
function checkAccess() {
  ...
  // Parse expiry — handle timestamp number, date string, or missing
  let expiry = 0;
  if (expRaw) {
    const n = parseInt(expRaw);
    expiry = isNaN(n) ? new Date(expRaw).getTime() : n;
    if (isNaN(expiry)) expiry = 0;
  }

  if (saved && expiry > now) {
    // Valid — boot app immediately
    hidePublicScreen();
    bootApp();
  } else if (saved) {
    // Has code but expiry missing or past — revalidate with server
    // Never delete the code locally — server decides
