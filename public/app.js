...
function checkAccess() {
  ...
  const expiry   = parseInt(localStorage.getItem('14t_access_exp') || '0');
  const now      = Date.now();
  // Valid if code matches and not expired
  if (saved === ACCESS_CODE && now < expiry) {
    unlockApp();
  } else {
...
async function fetchLivePriceForCountdown(asset) {
  try {
    ...
    if (asset === 'BTC') {
      ...
      priceEl.textContent = '$' + parseFloat(p).toLocaleString('en-US', {minimumFractionDigits:2, maximumFractionDigits:2});
...
function showSignalCountdown(prep) {
  ...
  const pairLabel = asset === 'BTC' ? 'BTC/USD' : 'XAU/USD';
  ...
  const l = levels[asset] || levels.GOLD;
  levelsEl.innerHTML = `
    <div class="scol-item"><div class="scol-lbl">SL</div><div class="scol-val sl">${l.sl}pts</div></div>
    <div class="scol-item"><div class="scol-lbl">TP1</div><div class="scol-val tp">+${l.tp1}pts</div></div>
    <div class="scol-item"><div class="scol-lbl">TP2</div><div class="scol-val tp2">+${l.tp2}pts</div></div>
    <div class="scol-item"><div class="scol-lbl">TP3</div><div class="scol-val tp3">+${l.tp3}pts</div></div>`;
...
navigator.serviceWorker && navigator.serviceWorker.addEventListener('message', event => {
  ...
  if (payload.type === 'broadcast') {
    showBroadcastMessage(payload.message);
    return;
  }
  if (payload.type === 'prep') {
    // Show countdown immediately from push data
    lastPrepId = null; // reset so it always shows
    showSignalCountdown({
...
function showBroadcastMessage(msg) {
  ...
  banner.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:800;background:#0A0A14;border-bottom:1px solid #4A9EFF30;padding:12px 18px;display:flex;align-items:center;gap:10px;animation:slide-down .3s ease';
  banner.innerHTML = `
    <div style="font-size:18px;flex-shrink:0">📢</div>
    <div style="flex:1">
      <div style="font-size:9px;font-weight:700;letter-spacing:1.5px;color:#4A9EFF;text-transform:uppercase;margin-bottom:3px">From 14 Trades</div>
      <div style="font-size:12px;color:#F0F0F0;line-height:1.4">${msg}</div>
    </div>
...
async function fetchLivePrices() {
  try {
    // BTC price
    const r1 = await fetch('https://api.coingecko.com/api/v3/simple/price?ids=bitcoin&vs_currencies=usd');
    const d1 = await r1.json();
    const btc = d1.bitcoin.usd;
    const btcEl = document.getElementById('mood-btc-price');
    if (btcEl) btcEl.textContent = '$' + Math.round(btc).toLocaleString('en-US');
  } catch(e) {}
  try {
    // Gold price
    const r2 = await fetch('https://api.metals.live/v1/spot/gold');
    const d2 = await r2.json();
    const gold = d2[0]?.price;
    const goldEl = document.getElementById('mood-gold-price');
    if (goldEl && gold) goldEl.textContent = '$' + parseFloat(gold).toFixed(2);
  } catch(e) {}
}
