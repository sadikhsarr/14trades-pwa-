...
function computeStatsFromSignals(signals) {
  ...
  const PIP_VALUES = { GOLD:10, SILVER:50, NAS100:10, US30:10, BTC:1 };
  const TP_PIPS    = { 1:100, 2:200, 3:350 };

  const fired  = signals.filter(s => s.type === 'signal');
  const tpHits = signals.filter(s => s.type === 'tp_hit');
  const slHits = signals.filter(s => s.type === 'sl_hit');
  const beHits = signals.filter(s => s.type === 'be_hit');

  const sigCount = fired.length;
  const winCount = tpHits.filter(s => s.outcome === 'win' || s.level > 0).length || tpHits.length;

  // P&L from tp/sl/be hits
  let pnl = 0;
  tpHits.forEach(s => {
    const pips = TP_PIPS[s.level] || 100;
    pnl += pips;
  });
  slHits.forEach(s => { pnl -= 120; });
  // be_hit = break even = 0 net

  // Today pips — only signals from today
  ...
  // Win rate
  const wr = sigCount > 0 ? Math.round((winCount / sigCount) * 100) : 0;

  // Update global vars + localStorage
  totalSigs  = sigCount;
  wins       = winCount;
  totalPnL   = pnl;
  todayPnL   = todayPips;
  localStorage.setItem('14t_signals',  sigCount);
  localStorage.setItem('14t_wins',     winCount);
  ...
  if (sigsEl) sigsEl.textContent = sigCount;
  ...
  let weekPips = 0;
  tpHits.filter(s => new Date(s.timestamp).getTime() >= weekTs).forEach(s => { weekPips += TP_PIPS[s.level]||100; });
  slHits.filter(s => new Date(s.timestamp).getTime() >= weekTs).forEach(() => { weekPips -= 120; });

  const weekSigs = weekTp + weekSl;
  if (weekSigs > 0) showDailyRecap(weekSigs, weekTp, weekPips);
}
...
function applyState(d) {
  ...
  // Hide/show secondary sections based on signal state
  function setSecondary(show) {
    ...
    document.querySelectorAll('[data-secondary]').forEach(el => { el.style.display=show?'':'none'; });
  }
