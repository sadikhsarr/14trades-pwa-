...
function updateStats() {
  ...
  if (lastState && Array.isArray(lastState.signals)) {
    ...
    if (outcomes.length > 0) {
      const liveWins = outcomes.filter(e => e.type === 'tp_hit' || e.type === 'be_hit').length;
...
async function loadWeeklyRecap() {
  ...
  try {
    ...
    const wins   = thisWeek.filter(e => e.type === 'tp_hit' || e.type === 'be_hit').length;
    ...
    if (progRow) {
      progRow.innerHTML = shown.length
        ? shown.map(e => {
            const isWin = e.type === 'tp_hit' || e.type === 'be_hit';
            return `<div style="flex:1;height:6px;border-radius:3px;background:${isWin?'#00C87A':'#2A1414'};${isWin?'box-shadow:0 0 12px #00C87A60':''}"></div>`;
          }).join('')
    ...
    if (progLbl) {
      progLbl.innerHTML = shown.length
        ? shown.map(e => {
            const isWin = e.type === 'tp_hit' || e.type === 'be_hit';
            return `<span style="${isWin?'':'color:#663333'}">${isWin?'Win':'Loss'}</span>`;
          }).join('')
