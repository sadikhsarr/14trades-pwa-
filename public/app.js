...
async function loadReferralBanner() {
  try {
    ...
    // Only show for member codes (not trial)
    const banner = document.getElementById('referral-banner');
    if (!banner) return;

    // Generate member ref code from their access code
    // Use first 6 alphanumeric chars — clean and readable
    const cleaned = code.replace(/[^A-Z0-9]/gi,'').toUpperCase();
    const refCode = 'REF-' + (cleaned.substring(0,6) || Math.random().toString(36).substr(2,6).toUpperCase());
    localStorage.setItem('14t_ref_code', refCode);

    // Show banner
    banner.style.display = 'flex';

    // Set ref code display
    ...
    if (txt)  txt.textContent  = (count % 3) + '/3';

    // Celebrate if just hit 3
    if (count > 0 && count % 3 === 0) {
      const txt2 = document.getElementById('ref-progress-txt');
      if (txt2) txt2.textContent = '3/3 🎉';
      if (fill) fill.style.background = '#30D158';
    }
...
function copyRefCode() {
  const code = localStorage.getItem('14t_ref_code');
  if (!code) return;
  const link = `https://app.14trade.com/trial.html?ref=${code}`;
